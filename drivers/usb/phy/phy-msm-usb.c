...
void otg_swic_ic_event(swic_accessory_enum event)
{
	...
	if (event == SWIC_USB_MODE) {
		swic_usb = 1;
        } else if (event == SWIC_AUDIO_CHG_MIC_MONO) {
                swic_usb = 1;
...
lpm_start:
	...
	floated_charger = motg->chg_type == USB_FLOATED_CHARGER;
        cradle_charger = motg->chg_type == USB_CRADLE_CHARGER;
	...
	if ((test_bit(B_SESS_VLD, &motg->inputs) && !device_bus_suspend &&
                !dcp && !prop_charger && !floated_charger && !cradle_charger) ||
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	if (motg->chg_type == USB_SDP_CHARGER)
        ...
        else if (motg->chg_type == USB_CRADLE_CHARGER)
                charger_type = POWER_SUPPLY_TYPE_USB_CRADLE;
...
static bool msm_chg_check_cradle(struct msm_otg *motg)
{
        bool ret = false;

        if(SWIC_AUDIO_CHG_MIC_MONO == swic_get_accessory()) {
                return true;
        }

        kc_dbg(motg->phy.dev, "= USB DET I = : %s = %s\n", __func__, ret?"true":"false");
        return ret;
}
...
static const char *chg_to_string(enum usb_chg_type chg_type)
{
	...
	case USB_FLOATED_CHARGER:	return "USB_FLOATED_CHARGER";
        case USB_CRADLE_CHARGER:	return "USB_CRADLE_CHARGER";
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout && !dm_vlgc) { /* VDAT_REF < DM < VLGC */
			...
			if (!dcd && floated_charger_enable){
				motg->chg_type = USB_FLOATED_CHARGER;
                                kc_dbg(motg->phy.dev, "= USB DET I = : chg_type = USB_FLOATED_CHARGER\n");
                                is_cradle = msm_chg_check_cradle(motg);
                                if (is_cradle) {
                                    motg->chg_type = USB_CRADLE_CHARGER;
                                    kc_dbg(motg->phy.dev, "= USB DET I = : chg_type = USB_CRADLE_CHARGER\n");
                                }
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_FLOATED_CHARGER:
					...
					pm_runtime_suspend(otg->phy->dev);
					break;
                                case USB_CRADLE_CHARGER:
                                        msm_otg_notify_charger(motg, IDEV_CHG_CRADLE);
                                        msm_otg_dbg_log_event(&motg->phy,
                                                "PM RUNTIME: CRADLE PUT",
                                                get_pm_runtime_counter(otg->phy->dev),
                                                0);
                                        pm_runtime_put_noidle(otg->phy->dev);
                                        pm_runtime_suspend(otg->phy->dev);
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	if ((SWIC_USB_MODE == swic_get_accessory()) || (SWIC_AUDIO_CHG_MIC_MONO == swic_get_accessory()) ) {
		boot_flg = 1;
	}
	swic_event = 0;
	swic_usb = 0;
