...
#define MSM_CHECK_WAKE_LOCK_DELAY (5 *HZ)
...
static int dcp_max_current = IDEV_CHG_MAX_DCP_CDP;
module_param(dcp_max_current, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(dcp_max_current, "max current drawn for DCP charger");

static bool wake_lock_enable_flag = false;
module_param(wake_lock_enable_flag , bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(wake_lock_enable_flag,"KC msm_otg Wake Lock Flag");

static uint force_suspend_counter = 0;
module_param(force_suspend_counter, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(force_suspend_counter, "Force Suspend counter");
...
static void msm_wake_lock_check_work(struct work_struct *w)
{
	struct msm_otg *motg = container_of(w, struct msm_otg, check_wake_lock_work.work);

	kc_dbg(motg->phy.dev, "msm_wake_lock_check_work: check start\n");

	if (wake_lock_enable_flag == false) {
		kc_dbg(motg->phy.dev, "msm_wake_lock_check_work: msm_otg wake_lock is already disabled\n");
		kc_dbg(motg->phy.dev, "msm_wake_lock_check_work: check end\n");
		return;
	} else {
		kc_dbg(motg->phy.dev, "msm_wake_lock_check_work: msm_otg wake_lock is enable \n");
		if (!test_bit(B_SESS_VLD, &motg->inputs) && motg->chg_state == USB_CHG_STATE_UNDEFINED) {
			pr_info("msm_wake_lock_check_work: vbus inactive. so force msm_otg suspend \n");
			msm_otg_suspend(motg);
			force_suspend_counter++;
			kc_dbg(motg->phy.dev, "msm_wake_lock_check_work: check end\n");
			return;
		} else {
			kc_dbg(motg->phy.dev, "msm_wake_lock_check_work: vbus active. so check continue\n");
		}
	}

	schedule_delayed_work(&motg->check_wake_lock_work, MSM_CHECK_WAKE_LOCK_DELAY);
}
...
#define MSM_CHG_DCD_TIMEOUT		(2500 * HZ/1000) /* 2500 msec */
		...
		if (!test_bit(ID, &motg->inputs) && otg->host) {
			...
			if (motg->chg_state == USB_CHG_STATE_UNDEFINED ||
				...
				msm_otg_dbg_log_event(&motg->phy, "RT !CHG GET",
				      get_pm_runtime_counter(otg->phy->dev), 0);
				if(get_pm_runtime_counter(otg->phy->dev) > 0){
					pr_err("Usage counter is greater than 1. so, pm_runtime_get_sync function is skip!!\n");
				} else {
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		kc_dbg(motg->phy.dev, "force_suspend_counter = %d\n", force_suspend_counter);
		if( !test_bit(B_SESS_VLD, &motg->inputs) && wake_lock_enable_flag == true) {
			kc_dbg(motg->phy.dev, "msm_wake_lock_check_work: work set\n");
			schedule_delayed_work(&motg->check_wake_lock_work, MSM_CHECK_WAKE_LOCK_DELAY);
		}
		break;
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&motg->check_wake_lock_work, msm_wake_lock_check_work);
