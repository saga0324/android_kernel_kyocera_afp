		...
		struct usb_configuration *c)
{
	...
	dev = gether_setup(c->cdev->gadget, dev_addr, host_addr,
							rndis->ethaddr, qmult);
					...
					struct usb_composite_dev *cdev)
{
	...
	set_vendor_sdev(fsg_opts->common, &dev->sdev);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size >= sizeof(config->inquiry_string))
		return -EINVAL;
	return strlcpy(config->inquiry_string, buf, (8 + 16 + 4 + 1));
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	fsg_opts = fsg_opts_from_func_inst(config->f_ms_inst);
	if (config == NULL || fsg_opts->common == NULL)
		return -EINVAL;

	if((set_vendor_cmd_1(buf, fsg_opts->common)) != 2){
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if((set_vendor_cmd_2(buf, fsg_opts->common)) != 2){
		return -EINVAL;
	}
...
static struct android_usb_function *supported_functions[] = {
	...
	[ANDROID_KC_SERIAL] = &kc_serial_function,
#ifndef FEATURE_KYOCERA_DATA_QCOM
	[ANDROID_NMEA] = &nmea_function,
#endif /* FEATURE_KYOCERA_DATA_QCOM */
			   ...
			   const char *buff, size_t size)
{
	...
	if (size >= sizeof(serial_string))
		return -EINVAL;
	if (strcmp("KYOCERA", serial_string)) {
