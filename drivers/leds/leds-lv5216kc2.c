...
#define LV5216_V_LOG(msg, ...)	\
	pr_notice("[LEDDRV]#2[%s][V](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)
#define LV5216_D_LOG(msg, ...)	\
	pr_notice("[LEDDRV]#2[%s][D](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)
#else
#define LV5216_V_LOG(msg, ...)
#define LV5216_D_LOG(msg, ...)	\
	pr_debug ("[LEDDRV]#2[%s][D](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)
#endif

#define LV5216_E_LOG(msg, ...)	\
	pr_err   ("[LEDDRV]#2[%s][E](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)
#define LV5216_N_LOG(msg, ...)	\
	pr_notice("[LEDDRV]#2[%s][N](%d): " msg "\n", __func__, __LINE__, ## __VA_ARGS__)


#define LV5216_DRV_NAME "LV5216_2"
...
static const uint8_t mled_reg03_reg01_manual2[256][2] = {
	{	0x00, 0x00	},	//0
	{	0x20, 0x1D	},	//1
	{	0x20, 0x1D	},	//2
	{	0x20, 0x1D	},	//3
...
static const uint8_t mled_reg03_reg01_manual3[256][2] = {
	{	0x00, 0x00	},	//0
	{	0x20, 0x1D	},	//1
	{	0x20, 0x1D	},	//2
	{	0x20, 0x1D	},	//3
	...
	struct lv5216_led_param *param_next)
{
	...
	if (param_next->value) {
			...
			(GET_CTRL_MODE(param_prev->ctrl_mode_mled) != CTRL_MODE_ALC &&
			 GET_CTRL_MODE(param_prev->ctrl_mode_mled) != CTRL_MODE_ALC_CAMERA)) {
				...
				if (sensor_brightness > max_level)
			 	...
			 	if(atomic_read(&g_panel_class) == 1){
					value = mled_reg03_reg01_manual2[sensor_brightness][0];
			 	}
			 	else if(atomic_read(&g_panel_class) == 2){
					value = mled_reg03_reg01_manual3[sensor_brightness][0];
			 	}
			 	else{
		...
		default:
			...
			if(atomic_read(&g_panel_class) == 1)
			{
				lv5216_i2c_masked_write(client, 0x03, mled_reg03_reg01_manual2[level][0], 0xFF);
				lv5216_i2c_masked_write(client, 0x01, mled_reg03_reg01_manual2[level][1], 0x1F);
			}
			else if(atomic_read(&g_panel_class) == 2)
			{
				lv5216_i2c_masked_write(client, 0x03, mled_reg03_reg01_manual3[level][0], 0xFF);
				lv5216_i2c_masked_write(client, 0x01, mled_reg03_reg01_manual3[level][1], 0x1F);
			}
			else
			{
	...
	while ((temp = of_get_next_child(node, temp))) {
		...
		if (led) {
			...
			if ( strcmp(led_label, LABEL_MLED) == 0 ) {
				...
				led->cdev.usr_brightness_req = wled_stat_aboot_level;
...
static int32_t __init lv5216_2_led_init(void)
{
	int32_t rc = 0;
...
static void __exit lv5216_2_led_exit(void)
{
	LV5216_V_LOG("[IN]");
...
static int __init lv5216_set_led_stat_aboot(char *buf)
{
	...
	if (ret) {
		...
		LV5216_N_LOG("[OUT] led_stat_aboot=0x%lx", led_stat_aboot);
...
int32_t lv5216_2_light_led_disp_set_panel(e_light_main_wled_disp disp_status, e_light_lcd_panel panel_class)
{
...
EXPORT_SYMBOL(lv5216_2_light_led_disp_set_panel);
...
int32_t lv5216_2_light_led_disp_power_set(e_light_main_wled_disp disp_status)
{
	struct lv5216_data *data = lv5216_data;
	struct lv5216_led_data *led;
	struct lv5216_led_param param_next;
...
EXPORT_SYMBOL(lv5216_2_light_led_disp_power_set);
...
module_init(lv5216_2_led_init);
module_exit(lv5216_2_led_exit);
