...
#define NUM_OF_COLORVARIATION	(3)
#define COLORVARIATION_DEFAULT	(0)
...
static const uint8_t keyled_current_setting[NUM_OF_COLORVARIATION][3] = {
	/* 06h(RC)	07h(GC)		08h(BC) */
	{0x04,		0x00,		0x01},		//RGB
	{0x05,		0x0C,		0x08},		//GB
	{0x00,		0x00,		0x00},		//RG
};
...
#define OEM_BOOT_LED_STAT_BL_LEVEL(level)  ((level&0xffff0000)>>16)
static int lv5216_led_stat_aboot = LV5216_LED_ABOOT_OFF;
static int wled_stat_aboot_level = 0;
/* Local Function */
#ifdef LIGHT_ALC_ENABLE
	...
	struct lv5216_led_param *param_next)
{
	...
	if (light_lcd_power_off()){
		if (led->cdev.sensor_brightness != 0) {
			LV5216_V_LOG("force backlight off");
			param_next->value = 0;
		} else {
			LV5216_V_LOG("discard light_lcd_power_off");
			goto exit;
		}
	}
	...
	if (param_next->value) {
...
#endif //LIGHT_ALC_ENABLE
			lv5216_i2c_masked_write(client, 0x01, 0x05, 0x07);
#ifdef LIGHT_ALC_ENABLE
			if (slope) {
				mutex_unlock(&data->control_lock);
		...
		case CTRL_MODE_ALC:
			...
			(GET_CTRL_MODE(param_prev->ctrl_mode_mled) != CTRL_MODE_ALC &&
			 GET_CTRL_MODE(param_prev->ctrl_mode_mled) != CTRL_MODE_ALC_CAMERA)) {
				...
				lv5216_i2c_masked_write(client, 0x01, 0x07, 0x07);
		...
		default:
			...
			lv5216_i2c_masked_write(client, 0x01, mled_reg03_reg01_manual[level][1], 0x07);
#ifdef LIGHT_ALC_ENABLE
			lv5216_i2c_masked_write(client, 0x14, 0x00, 0x20);
			lv5216_photo_sensor_control(data, led->led_type, false);
#endif //LIGHT_ALC_ENABLE
		...
		lv5216_i2c_masked_write(client, 0x0B, 0x00, 0x38);
#endif //LIGHT_ALC_ENABLE
		lv5216_i2c_masked_write(client, 0x01, 0x00, 0x07);
#ifdef LIGHT_ALC_ENABLE
		lv5216_photo_sensor_control(data, led->led_type, false);
#endif //LIGHT_ALC_ENABLE
...
static void lv5216_rgb_off(struct lv5216_data *data)
{
	...
	gpio_set_value(data->green_led_gpio, 0);

	lv5216_led_stat_aboot = LV5216_LED_ABOOT_OFF;
	LV5216_V_LOG("[OUT]");
}

static void lv5216_rgb_on(struct lv5216_data *data, uint32_t value)
{
	struct i2c_client *client = data->client;
	uint8_t rgb_on;
	int color;
	...
	if (rgb_on & 0x02) {
		gpio_set_value(data->green_led_gpio, 1);
	} else if ((rgb_on & 0x04)&&(!(rgb_on & 0x01))) {
		gpio_set_value(data->green_led_gpio, 1);
	} else {
		gpio_set_value(data->green_led_gpio, 0);
	}
...
static void lv5216_rgb_blink(struct lv5216_data *data, struct lv5216_led_param *param)
{
	...
	if (blink) {
		...
		lv5216_power_control(data, LED_TYPE_RGB, true);
		lv5216_rgb_on(data, param->value);
	}
	...
	struct lv5216_led_param *param_next)
{
	...
	if ((param_next->value & LIGHT_RGB_MASK)) {
		...
		rgb_on = ((param_next->value & 0x00ff0000) ? 0x08 : 0) |
				 ((param_next->value & 0x0000ff00) ? 0x10 : 0) |
				 ((param_next->value & 0x000000ff) ? 0x00 : 0);

		lv5216_power_control(data, led->led_type, true);
		lv5216_i2c_masked_write(client, 0x01, rgb_on, 0x18);
	} else {
		lv5216_i2c_masked_write(client, 0x01, 0x00, 0x18);
		lv5216_power_control(data, led->led_type, false);
	}
...
#ifdef LIGHT_KEYLED_GPIO
static void lv5216_keyled_current_write(struct i2c_client *client, int color)
{
	struct lv5216_data *data = i2c_get_clientdata(client);
	const uint8_t *reg_color;
	LV5216_V_LOG("[IN] colvar:%d color:%d",data->colorvariation, color);

	if (color >= 0 && color < NUM_OF_COLORVARIATION) {
		reg_color = &keyled_current_setting[data->colorvariation][0];
		lv5216_i2c_masked_write(client, 0x06, *reg_color, 0x1F);
		lv5216_i2c_masked_write(client, 0x07, *(reg_color+1), 0x1F);
		lv5216_i2c_masked_write(client, 0x08, *(reg_color+2), 0x1F);
	}
	...
	struct lv5216_led_param *param_next)
{
	...
	if ((param_next->value & LIGHT_RGB_MASK)) {
		...
		if (!(param_next->value & LIGHT_FACT_MODE_MASK)) {
			if (led->parent->colorvariation == 0) {
				rgb_on =0x05;
			} else if (led->parent->colorvariation == 1) {
				rgb_on =0x07;
			} else {
				rgb_on =0x03;
			}
		...
		lv5216_power_control(data, led->led_type, true);
		lv5216_keyled_current_write(client, led->parent->colorvariation);
...
static void lv5216_blink_work(struct work_struct *work)
{
	...
	if (!data->blink_ctl.next) {
		data->blink_ctl.next = true;
		lv5216_rgb_on(data, data->blink_ctl.param.blink_off_color);
		...
		lv5216_rgb_on(data, data->blink_ctl.param.value);
		queue_delayed_work(led->workqueue, &data->blink_work,
...
static int lv5216_remove(struct i2c_client *client)
{
	...
	if (data) {
		...
		if (gpio_is_valid(data->green_led_gpio)) {
			gpio_free(data->green_led_gpio);
		}
...
#endif
	lv5216_i2c_masked_write(client, 0x04, 0x00, 0x00);
	lv5216_i2c_masked_write(client, 0x05, 0x00, 0x00);
				   ...
				   const struct i2c_device_id *id)
{
	...
	while ((temp = of_get_next_child(node, temp))) {
		...
		if (strcmp(led_label, LABEL_MLED) == 0) {
			...
			data->green_led_gpio = of_get_named_gpio(client->dev.of_node, "kc,green-led-gpio", 0);
		...
		if (led) {
			...
			if ( strcmp(led_label, LABEL_MLED) == 0 ) {
				led->cdev.brightness = wled_stat_aboot_level;
	...
	if (gpio_is_valid(data->green_led_gpio)) {
		err = gpio_request(data->green_led_gpio, LV5216_DRV_NAME);
		LV5216_V_LOG("##gpio_request GPIO=%d err=%d", data->green_led_gpio, err);
		if (err < 0) {
			LV5216_E_LOG("failed to request GPIO=%d, ret=%d",
					data->green_led_gpio,
					err);
			goto fail_request_green_led_gpio;
		}
	}
...
#ifdef LB5216KC_BOOT_BL
static int __init lv5216_set_led_stat_aboot(char *buf)
{
	int ret;
	unsigned long led_stat_aboot = BOOT_LED_STAT_OFF;

	LV5216_V_LOG("[IN]");

	ret = kstrtoul(buf, 0, &led_stat_aboot);
	if (ret) {
		LV5216_V_LOG("failed to kstrtoul ret=%d", ret);
	} else {
		if (led_stat_aboot&BOOT_LED_STAT_3COLOR_ON) {
			lv5216_led_stat_aboot = LV5216_LED_ABOOT_ON;
		}
		if (led_stat_aboot&BOOT_LED_STAT_BL_ON) {
			wled_stat_aboot_level = OEM_BOOT_LED_STAT_BL_LEVEL(led_stat_aboot);
		} else {
			wled_stat_aboot_level = 0;
		}
		LV5216_V_LOG("[OUT] led_stat_aboot=0x%lx wled_stat_aboot_level=%d", led_stat_aboot, wled_stat_aboot_level);
	}
	return 0;
}
