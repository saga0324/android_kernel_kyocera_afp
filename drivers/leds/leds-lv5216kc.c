...
#define NV_BK_LIGHT_CTL_LOWER_L_I_DEF	(0x03)
...
#define LABEL_GLIGHTCA	"glightcall"
#define LABEL_GLIGHTSK1	"glightsk1"
#define LABEL_GLIGHTSK2	"glightsk2"
#define LABEL_GLIGHTCSK	"glightcsk"
...
enum {
	...
	LED_TYPE_KEYLED,
	LED_TYPE_SLED,
	LED_TYPE_GUIDELIGHT_CALL,
	LED_TYPE_GUIDELIGHT_SK1,
	LED_TYPE_GUIDELIGHT_SK2,
	LED_TYPE_GUIDELIGHT_CSK,
	LED_TYPE_MAX
};
...
struct lv5216_data {
	...
	int							vkey_gpio;
	int							sled_gpio;
	int							guidelight_call_gpio;
	int							guidelight_sk1_gpio;
	int							guidelight_sk2_gpio;
	int							guidelight_csk_gpio;
	...
	int							colorvariation;
	struct pinctrl				*guidelight_pinctrl;
...
struct guidelight_data {
	int							head;
	bool						blink_ctrl_next;
	struct guidelight_param		data[GUIDELIGHT_TYPE_MAX];
};
static struct guidelight_data guidelight_ctrl;
...
#define LV5216_LED_ABOOT_ON		(1)
#define LV5216_LED_ABOOT_OFF	(0)
static int lv5216_led_stat_aboot = LV5216_LED_ABOOT_OFF;
...
static void guidelight_blink_timer_cb(struct work_struct *);
static DECLARE_DELAYED_WORK(guidelight_blink_timer, guidelight_blink_timer_cb);
...
static void lv5216_vkey_control(struct lv5216_data *data, int led_type, bool on)
{
	int state_prev = data->vkey_state;
	int state_next = data->vkey_state;
	...
	if ((state_prev == 0) && (0 != state_next)) {
		gpio_set_value(data->vkey_gpio, 1);
		usleep(2000);
	} else if ((state_prev != 0) && (0 == state_next)) {
		gpio_set_value(data->vkey_gpio, 0);
		usleep(2000);
	} else {
		LV5216_D_LOG("no change");
	}

	data->vkey_state = state_next;
...
static void lv5216_rgb_off(struct lv5216_data *data)
{
	...
	lv5216_led_stat_aboot = LV5216_LED_ABOOT_OFF;
	LV5216_V_LOG("[OUT]");
}
...
static void lv5216_guidelight_off(struct lv5216_data *data, int led_type)
{
	LV5216_V_LOG("[IN]");

	LV5216_D_LOG("Off (%d)", led_type);
	if( GUIDELIGHT_TYPE_CALL == led_type) {
		if (gpio_is_valid(data->guidelight_call_gpio)) {
			gpio_set_value(data->guidelight_call_gpio, 0);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_CALL GPIO specified %d", data->guidelight_call_gpio);
		}
	}
	if( GUIDELIGHT_TYPE_SK1 == led_type) {
		if (gpio_is_valid(data->guidelight_sk1_gpio)) {
			gpio_set_value(data->guidelight_sk1_gpio, 0);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_SK1 GPIO specified %d", data->guidelight_sk1_gpio);
		}
	}
	if( GUIDELIGHT_TYPE_SK2 == led_type) {
		if (gpio_is_valid(data->guidelight_sk2_gpio)) {
			gpio_set_value(data->guidelight_sk2_gpio, 0);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_SK2 GPIO specified %d", data->guidelight_sk2_gpio);
		}
	}
	if( GUIDELIGHT_TYPE_CSK == led_type) {
		if (gpio_is_valid(data->guidelight_csk_gpio)) {
			gpio_set_value(data->guidelight_csk_gpio, 0);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_CSK GPIO specified %d", data->guidelight_csk_gpio);
		}
	}

	lv5216_vkey_control(data, led_type, false);

	LV5216_V_LOG("[OUT]");
	return;
}

static void lv5216_guidelight_on(struct lv5216_data *data,int led_type, uint32_t value)
{
	int color = 0;

	LV5216_V_LOG("[IN] value=0x%08x", value);

	lv5216_vkey_control(data, led_type, true);

	color = ((value & 0x00ffffff) ? 1 : 0);

	LV5216_D_LOG("On (%d): %d", led_type, color);
	if( GUIDELIGHT_TYPE_CALL == led_type) {
		if (gpio_is_valid(data->guidelight_call_gpio)) {
			gpio_set_value(data->guidelight_call_gpio, color);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_CALL GPIO specified %d", data->guidelight_call_gpio);
		}
	}
	if( GUIDELIGHT_TYPE_SK1 == led_type) {
		if (gpio_is_valid(data->guidelight_sk1_gpio)) {
			gpio_set_value(data->guidelight_sk1_gpio, color);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_SK1 GPIO specified %d", data->guidelight_sk1_gpio);
		}
	}
	if( GUIDELIGHT_TYPE_SK2 == led_type) {
		if (gpio_is_valid(data->guidelight_sk2_gpio)) {
			gpio_set_value(data->guidelight_sk2_gpio, color);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_SK2 GPIO specified %d", data->guidelight_sk2_gpio);
		}
	}
	if( GUIDELIGHT_TYPE_CSK == led_type) {
		if (gpio_is_valid(data->guidelight_csk_gpio)) {
			gpio_set_value(data->guidelight_csk_gpio, color);
		} else {
			LV5216_E_LOG("No valid GUIDELIGHT_CSK GPIO specified %d", data->guidelight_csk_gpio);
		}
	}
...
static void lv5216_guidelight_blink_work(struct work_struct *work)
{
	struct lv5216_data *data = lv5216_data;
	int i;
	int blink = GUIDELIGHT_TYPE_MAX;

	LV5216_V_LOG("[IN]");
	mutex_lock(&data->control_lock);

	if(delayed_work_pending(&guidelight_blink_timer)){
		mutex_unlock(&data->control_lock);
		return;
	}

	if(!guidelight_ctrl.blink_ctrl_next){
		LV5216_V_LOG("ON -> OFF");
		for( i = 0; i < GUIDELIGHT_TYPE_MAX; i++ ){
			if( guidelight_ctrl.data[i].blink ){
				lv5216_guidelight_on(data, i, guidelight_ctrl.data[i].blink_off_color);
				blink = i;
			}
		}
		if(blink < GUIDELIGHT_TYPE_MAX){
			guidelight_ctrl.blink_ctrl_next = true;
			schedule_delayed_work(&guidelight_blink_timer, msecs_to_jiffies(guidelight_ctrl.data[blink].blink_low_pause_time));
		}
	} else {
		LV5216_V_LOG("OFF -> ON");
		for( i = 0; i < GUIDELIGHT_TYPE_MAX; i++ ){
			if( guidelight_ctrl.data[i].blink ){
				lv5216_guidelight_on(data, i, guidelight_ctrl.data[i].color);
				blink = i;
			}
		}
		if(blink < GUIDELIGHT_TYPE_MAX){
			guidelight_ctrl.blink_ctrl_next = false;
			schedule_delayed_work(&guidelight_blink_timer, msecs_to_jiffies(guidelight_ctrl.data[blink].blink_high_pause_time));
		}
	}
	mutex_unlock(&data->control_lock);
...
static void guidelight_blink_timer_cb(struct work_struct *work)
{
	LV5216_V_LOG("[IN]");
	lv5216_guidelight_blink_work(work);
	LV5216_V_LOG("[OUT]");
}

static void lv5216_guidelight_blink(struct lv5216_data *data, struct lv5216_led_param *param, int led_type)
{
	int blink;
	int i;

	LV5216_V_LOG("[IN] ledtype=%d", led_type);

	guidelight_ctrl.head = led_type;
	guidelight_ctrl.data[led_type].color = param->value & 0x00ffffff;
	guidelight_ctrl.data[led_type].blink = ((param->value && param->blink_high_pause_time && param->blink_low_pause_time) ? 1 : 0);
	guidelight_ctrl.data[led_type].blink_high_pause_time = param->blink_high_pause_time;
	guidelight_ctrl.data[led_type].blink_low_pause_time = param->blink_low_pause_time;
	guidelight_ctrl.data[led_type].blink_off_color = param->blink_off_color;

	for( i = 0; i < GUIDELIGHT_TYPE_MAX; i++ ){
		LV5216_D_LOG("GUIDELIGHT(%d)%s col=0x%08x blink=%d On=%d Off=%d off_col=0x%08x",
			i,
			((i == guidelight_ctrl.head) ? "*" : " "),
			guidelight_ctrl.data[i].color,
			guidelight_ctrl.data[i].blink,
			guidelight_ctrl.data[i].blink_high_pause_time,
			guidelight_ctrl.data[i].blink_low_pause_time,
			guidelight_ctrl.data[i].blink_off_color);
	}

	guidelight_ctrl.blink_ctrl_next = false;
	cancel_delayed_work(&guidelight_blink_timer);
	lv5216_guidelight_off(data, led_type);

	if (param->value) {
		if (!guidelight_ctrl.data[led_type].blink) {
			lv5216_guidelight_on(data, led_type, guidelight_ctrl.data[led_type].color);
		}
	}

	/* blink */
	blink = GUIDELIGHT_TYPE_MAX;
	for( i = 0; i < GUIDELIGHT_TYPE_MAX; i++ ){
		if( guidelight_ctrl.data[i].blink ){
			blink = i;
			break;
	...
	if( blink < GUIDELIGHT_TYPE_MAX ){
		for( i = 0; i < GUIDELIGHT_TYPE_MAX; i++ ){
			if( guidelight_ctrl.data[i].blink ){
				LV5216_V_LOG("blink type=%d", i);
				lv5216_guidelight_on(data, i, guidelight_ctrl.data[i].color);
			}
		}
		LV5216_D_LOG("blink On=%d Off=%d",
				guidelight_ctrl.data[blink].blink_high_pause_time, guidelight_ctrl.data[blink].blink_low_pause_time);
		schedule_delayed_work(&guidelight_blink_timer, msecs_to_jiffies(guidelight_ctrl.data[blink].blink_high_pause_time));
	}
...
static void lv5216_ctrl_guidelight_call(struct lv5216_led_data *led,
	struct lv5216_led_param *param_next)
{
	struct lv5216_data *data = led->parent;

	LV5216_V_LOG("[IN] value=0x%08x", param_next->value);
	lv5216_guidelight_blink(data, param_next, GUIDELIGHT_TYPE_CALL);
	LV5216_V_LOG("[OUT]");
}


static void lv5216_ctrl_guidelight_sk1(struct lv5216_led_data *led,
	struct lv5216_led_param *param_next)
{
	struct lv5216_data *data = led->parent;

	LV5216_V_LOG("[IN] value=0x%08x", param_next->value);
	lv5216_guidelight_blink(data, param_next, GUIDELIGHT_TYPE_SK1);
	LV5216_V_LOG("[OUT]");
}

static void lv5216_ctrl_guidelight_sk2(struct lv5216_led_data *led,
	struct lv5216_led_param *param_next)
{
	struct lv5216_data *data = led->parent;

	LV5216_V_LOG("[IN] value=0x%08x", param_next->value);
	lv5216_guidelight_blink(data, param_next, GUIDELIGHT_TYPE_SK2);
	LV5216_V_LOG("[OUT]");
}

static void lv5216_ctrl_guidelight_csk(struct lv5216_led_data *led,
	struct lv5216_led_param *param_next)
{
	struct lv5216_data *data = led->parent;

	LV5216_V_LOG("[IN] value=0x%08x", param_next->value);
	lv5216_guidelight_blink(data, param_next, GUIDELIGHT_TYPE_CSK);
	LV5216_V_LOG("[OUT]");
}
...
#define LEDLIGHT_PARAM_GLIGHT   (5)
...
static long leds_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case LEDLIGHT_SET_PARAM:
		...
		case LEDLIGHT_PARAM_GLIGHT:
			...
			param->blink_off_color       = st_ioctl.data[4];
			mutex_unlock(&data->param_lock);
			LV5216_D_LOG("LEDLIGHT_PARAM_GUIDELIGHT blink_control=%d high=%d low=%d off_color=0x%08x",
				param->blink_control, param->blink_high_pause_time,
				param->blink_low_pause_time, param->blink_off_color);
			break;
		default:
...
static int lv5216_init_pinctrl(struct lv5216_data *data)
{
	int retval;

	LV5216_V_LOG("[IN]");

	data->guidelight_pinctrl = devm_pinctrl_get(&(data->client->dev));
	if (IS_ERR_OR_NULL(data->guidelight_pinctrl)) {
		LV5216_E_LOG("Target does not use pinctrl");
		retval = PTR_ERR(data->guidelight_pinctrl);
		data->guidelight_pinctrl = NULL;
		return retval;
	}

	data->gpio_state_active
		= pinctrl_lookup_state(data->guidelight_pinctrl, "guidelight_active");
	if (IS_ERR_OR_NULL(data->gpio_state_active)) {
		LV5216_E_LOG("Can not get key_led active(default) pinstate");
	...
	data->gpio_state_suspend
		= pinctrl_lookup_state(data->guidelight_pinctrl, "guidelight_suspend");
	if (IS_ERR_OR_NULL(data->gpio_state_suspend)) {
		LV5216_E_LOG("Can not get key_led suspend(sleep) pinstate");
...
static int lv5216_pinctrl_select(struct lv5216_data *data, bool on)
{
	struct pinctrl_state *pins_state;
	int ret = -1;

	LV5216_V_LOG("[IN] set '%s' pins state", on ? "guidelight_active" : "guidelight_suspend");

	pins_state = on ? data->gpio_state_active : data->gpio_state_suspend;
	if(!IS_ERR_OR_NULL(pins_state)){
		ret = pinctrl_select_state(data->guidelight_pinctrl, pins_state);
		if(ret){
			LV5216_E_LOG("can not set '%s' pins state", on ? "guidelight_active" : "guidelight_suspend");
		}
	}else{
		LV5216_E_LOG("not a valid(err or null)");
	}
	LV5216_V_LOG("[OUT]");
	return ret;
}
...
static int lv5216_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	ret = lv5216_pinctrl_select(data, false);
	if(ret){
		LV5216_E_LOG("Failed LV5216_pinctrl_select false");
	}
...
static int lv5216_resume(struct i2c_client *client)
{
	...
	ret = lv5216_pinctrl_select(data, true);
	if(ret){
		LV5216_E_LOG("Failed LV5216_pinctrl_select true");
	}
	LV5216_V_LOG("[OUT]");
	return ret;
}
...
static int lv5216_remove(struct i2c_client *client)
{
	...
	if (data) {
		...
		if (gpio_is_valid(data->vkey_gpio)) {
			gpio_free(data->vkey_gpio);
		}
				   ...
				   const struct i2c_device_id *id)
{
	...
	data->vkey_gpio = -ENOSYS;
	data->sled_gpio = -ENOSYS;
	data->guidelight_call_gpio = -ENOSYS;
	data->guidelight_sk1_gpio = -ENOSYS;
	data->guidelight_sk2_gpio = -ENOSYS;
	data->guidelight_csk_gpio = -ENOSYS;
	data->reset_gpio = -ENOSYS;
	temp = NULL;
	while ((temp = of_get_next_child(node, temp))) {
		...
		if (strcmp(led_label, LABEL_MLED) == 0) {
			...
			data->vkey_gpio = of_get_named_gpio(client->dev.of_node, "kc,vkey-gpio", 0);
			if (!gpio_is_valid(data->vkey_gpio)) {
				LV5216_E_LOG("No valid VKEY GPIO specified %d", data->vkey_gpio);
				goto fail_id_check;
			}
			data->vkey_state = 0;
		...
		} else if (strcmp(led_label, LABEL_GLIGHTCA) == 0) {
			LV5216_V_LOG("probe GUIDELIGHT_CALL");
			led = &data->led_data[LED_TYPE_GUIDELIGHT_CALL];
			led->led_type = LED_TYPE_GUIDELIGHT_CALL;
			led->ctrl_func = lv5216_ctrl_guidelight_call;
			led->workqueue = system_wq;
			data->guidelight_call_gpio = of_get_named_gpio(client->dev.of_node, "kc,guidelight_call-gpio", 0);
			if (!gpio_is_valid(data->guidelight_call_gpio)) {
				LV5216_E_LOG("No valid GUIDELIGHT_CALL GPIO specified %d", data->guidelight_call_gpio);
				goto fail_id_check;
			}
		} else if (strcmp(led_label, LABEL_GLIGHTSK1) == 0) {
			LV5216_V_LOG("probe GUIDELIGHT_SK1");
			led = &data->led_data[LED_TYPE_GUIDELIGHT_SK1];
			led->led_type = LED_TYPE_GUIDELIGHT_SK1;
			led->ctrl_func = lv5216_ctrl_guidelight_sk1;
			led->workqueue = system_wq;
			data->guidelight_sk1_gpio = of_get_named_gpio(client->dev.of_node, "kc,guidelight_sk1-gpio", 0);
			if (!gpio_is_valid(data->guidelight_sk1_gpio)) {
				LV5216_E_LOG("No valid GUIDELIGHT_SK1 GPIO specified %d", data->guidelight_sk1_gpio);
				goto fail_id_check;
			}
		} else if (strcmp(led_label, LABEL_GLIGHTSK2) == 0) {
			LV5216_V_LOG("probe GUIDELIGHT_SK2");
			led = &data->led_data[LED_TYPE_GUIDELIGHT_SK2];
			led->led_type = LED_TYPE_GUIDELIGHT_SK2;
			led->ctrl_func = lv5216_ctrl_guidelight_sk2;
			led->workqueue = system_wq;
			data->guidelight_sk2_gpio = of_get_named_gpio(client->dev.of_node, "kc,guidelight_sk2-gpio", 0);
			if (!gpio_is_valid(data->guidelight_sk2_gpio)) {
				LV5216_E_LOG("No valid GUIDELIGHT_SK2 GPIO specified %d", data->guidelight_sk2_gpio);
				goto fail_id_check;
			}
		} else if (strcmp(led_label, LABEL_GLIGHTCSK) == 0) {
			LV5216_V_LOG("probe GUIDELIGHT_CSK");
			led = &data->led_data[LED_TYPE_GUIDELIGHT_CSK];
			led->led_type = LED_TYPE_GUIDELIGHT_CSK;
			led->ctrl_func = lv5216_ctrl_guidelight_csk;
			led->workqueue = system_wq;
			data->guidelight_csk_gpio = of_get_named_gpio(client->dev.of_node, "kc,guidelight_csk-gpio", 0);
			if (!gpio_is_valid(data->guidelight_csk_gpio)) {
				LV5216_E_LOG("No valid GUIDELIGHT_CSK GPIO specified %d", data->guidelight_csk_gpio);
				goto fail_id_check;
			}
	...
	if (gpio_is_valid(data->vkey_gpio)) {
		err = gpio_request(data->vkey_gpio, LV5216_DRV_NAME);
		LV5216_V_LOG("gpio_request GPIO=%d err=%d", data->vkey_gpio, err);
		if (err < 0) {
			LV5216_E_LOG("failed to request GPIO=%d, ret=%d",
					data->vkey_gpio,
					err);
			goto fail_request_vkey_gpio;
		}
	}
	...
	err = lv5216_init_pinctrl(data);
	if(err){
		LV5216_E_LOG("Failed LV5216_init_pinctrl");
		goto fail_init_pinctrl;
	}

	err = lv5216_pinctrl_select(data, true);
	if(err){
		LV5216_E_LOG("Failed LV5216_pinctrl_select true");
		goto fail_pinctrl_select;
	}
...
static int __init lv5216_set_led_stat_aboot(char *buf)
{
	LV5216_V_LOG("[IN]");
	lv5216_led_stat_aboot = LV5216_LED_ABOOT_ON;
	LV5216_V_LOG("[OUT]");
	return 0;
}
early_param("led_stat", lv5216_set_led_stat_aboot);
