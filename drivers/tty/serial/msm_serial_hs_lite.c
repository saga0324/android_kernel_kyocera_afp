...
static const char device_name_hsl2[64] = {0x74, 0x74, 0x79, 0x48, 0x53, 0x4C, 0x33};
...
#ifdef FEATURE_KC_UART_WRITE_SYNC2
	if (strcmp(port->state->port.tty->name , device_name_hsl2) == 0 && msm_hsl_port->is_irda != true)
	{
...
static void msm_hsl_get_pinctrl_configs(struct uart_port *uport)
{
	...
	{
	case 3:
	    c_inactive = "uart3_inactive";
	    c_active = "uart3_active";
	    c_sleep = "uart3_sleep";
		break;
	default:
		return;
...
static int msm_hsl_startup(struct uart_port *port)
{
    ...
    /* disable irda tranceiver */
	msm_hsl_write(port, 0x00, regmap[msm_hsl_port->ver_id][UARTDM_IRDA]);
    msm_hsl_port->is_irda = false;
...
static int msm_hsl_ioctl(struct uart_port *uport, unsigned int cmd,
						unsigned long arg)
{
    int ret = 0;
    ...
    switch(cmd){
    case TIOCSERIROFF:
        pr_info("IrDA Tranceiver off\n");
	    spin_lock_irqsave(&uport->lock, flags);
        msm_hsl_write(uport, 0x00, regmap[vid][UARTDM_IRDA]);
        msm_hsl_port->is_irda = false;
	    spin_unlock_irqrestore(&uport->lock, flags);
        break;
    case TIOCSERIRON:
        pr_info("IrDA Tranceiver on\n");
	    spin_lock_irqsave(&uport->lock, flags);
        msm_hsl_write(uport, 0x03, regmap[vid][UARTDM_IRDA]);
        msm_hsl_port->is_irda = true;
	    spin_unlock_irqrestore(&uport->lock, flags);
        break;
    default:
