...
static ssize_t acc_lnr_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t acc_lnr_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* acc_lnr_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    acc_lnr_batch_hal_addr = data_addr;

    SENSOR_N_LOG("acc_lnr_batch_hal_addr[%x]", (int)acc_lnr_batch_hal_addr);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_ACC_LNR,acc_lnr_batch_data,acc_lnr_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
...
static void acc_lnr_poll_work_func(struct work_struct *work)
{
    ...
    polltime = atomic_read(&(acc_lnr_poll_info.poll_time));
    if (polltime > 0) {
        queue_delayed_work( (acc_lnr_poll_info.poll_wq),
                            &(acc_lnr_poll_info.poll_work),
                            msecs_to_jiffies(polltime) );
        SENSOR_A_LOG("start delay work :polltime[%d]",
                     atomic_read(&(acc_lnr_poll_info.poll_time)) );
    } else {
...
void acc_lnr_driver_init( void )
{
    ...
    SENSOR_N_LOG("sensor_input_init()-->ret[%d] dev[%d]",
                  ret, (int)(acc_lnr_input_info.dev) );
