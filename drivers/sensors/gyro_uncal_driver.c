...
static ssize_t gyro_uncal_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t gyro_uncal_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* gyro_uncal_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    gyro_uncal_batch_hal_addr = data_addr;

    SENSOR_N_LOG("gyro_uncal_batch_hal_addr[%x]", (int)gyro_uncal_batch_hal_addr);

    if(sensor_copy_batch_uncal_data(SENSOR_BATCH_TBL_GYRO_UNCAL,gyro_uncal_batch_data,
                              gyro_uncal2_batch_data, gyro_uncal_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
...
static void gyro_uncal_poll_work_func(struct work_struct *work)
{
    ...
    polltime = atomic_read(&(gyro_uncal_poll_info.poll_time));
    if (polltime > 0) {
        queue_delayed_work( (gyro_uncal_poll_info.poll_wq),
                            &(gyro_uncal_poll_info.poll_work),
                            msecs_to_jiffies(polltime) );
        SENSOR_A_LOG("start delay work :polltime[%d]",
                     atomic_read(&(gyro_uncal_poll_info.poll_time)) );
    } else {
...
void gyro_uncal_driver_init( void )
{
    ...
    SENSOR_N_LOG("sensor_input_init()-->ret[%d] dev[%d]",
                  ret, (int)(gyro_uncal_input_info.dev) );
