...
static struct acc_if_funcs acc_func;
...
static void alps_func_register(void);
...
static long alps_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ALPSIO_SET_ACCACTIVATE:
		...
		dev_dbg(&adev->idev->dev, ALPS_IDEV_LOG_TAG
			...
			tmpval);
		mutex_lock(&alps_lock);
	    acc_func.activate(1, tmpval, adev->delay);
		if (!adev->flgM && !adev->flgA && tmpval)
			hrtimer_start(&adev->timer,
				ns_to_ktime((u64)adev->delay * NSEC_PER_MSEC),
				HRTIMER_MODE_REL);
	...
	case ALPSIO_SET_DELAY:
		...
		adev->delay = tmpval;
		acc_func.activate(1, adev->flgA, adev->delay);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    dev_dbg(&adev->idev->dev, ALPS_IDEV_LOG_TAG
        "enable %d\n",(int)enable);
    mutex_lock(&alps_lock);
    acc_func.activate(1, (int)enable, adev->delay);
...
static void alps_func_register(void)
{
    ...
    accsns_func_register_u2dh(&acc_func);
#elif defined (CONFIG_INPUT_SENSOR_ACC_MXC400XXC)
    accsns_func_register_mxc400xxc(&acc_func);
#endif
}
