...
static ssize_t mag_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t mag_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* mag_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    mag_batch_hal_addr = data_addr;

    SENSOR_N_LOG("mag_batch_hal_addr[%x]", (int)mag_batch_hal_addr);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_MAG,mag_batch_data,mag_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
    ...
    char *buf )
{
    ...
    SENSOR_N_LOG("sns_mag_get_accuracy ret[%d]",ret);
    ...
    char *buf )
{
    ...
    if(ret == SNS_RC_OK) {
        mag_offset.calib_offset.v[0] = offsets[0];
        mag_offset.calib_offset.v[1] = offsets[1];
        mag_offset.calib_offset.v[2] = offsets[2];
   ...
   size_t count )
{
    ...
    offsets[0] = mag_offset.calib_offset.v[0];
    offsets[1] = mag_offset.calib_offset.v[1];
    offsets[2] = mag_offset.calib_offset.v[2];

    sensor_com_mutex_lock();
    ret = sns_mag_set_offset(offsets);

    ret = sns_mag_set_accuracy((int8_t)mag_last_status);
    sensor_com_mutex_unlock();
...
static void mag_poll_work_func(struct work_struct *work)
{
    ...
    polltime = atomic_read(&(mag_poll_info.poll_time));
    if (polltime > 0) {
        queue_delayed_work( (mag_poll_info.poll_wq),
                            &(mag_poll_info.poll_work),
                            msecs_to_jiffies(polltime) );
        SENSOR_A_LOG("start delay work :polltime[%d]",
                     atomic_read(&(mag_poll_info.poll_time)) );
    } else {
