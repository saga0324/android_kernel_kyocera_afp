...
#define SENSOR_GPIO_VSENSOR_VDD		(31)
#endif
...
#ifdef CONFIG_USE_VSENSORVDD
	rc = gpio_request(SENSOR_GPIO_VSENSOR_VDD, DRV_NAME);
    if (rc < 0)
    {
        pr_err("[SP]failed to request GPIO[%d] ret[%d]\n",SENSOR_GPIO_VSENSOR_VDD,rc);
        return -EINVAL;
    }

    rc = gpio_direction_output(SENSOR_GPIO_VSENSOR_VDD, 1);
    if (rc < 0){
        pr_err("[SP]failed to gpio_direction_output SENSOR_GPIO_VSENSOR_VDD[%d]",rc);
        rc = -EINVAL;
		goto err;
    }
#endif
...
#ifdef CONFIG_USE_VSENSORVDD
err:
	gpio_free(SENSOR_GPIO_VSENSOR_VDD);
#endif
#if 0
	if (vpro_vreg)
