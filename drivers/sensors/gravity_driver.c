...
static ssize_t grv_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t grv_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* grv_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    grv_batch_hal_addr = data_addr;

    SENSOR_N_LOG("grv_batch_hal_addr[%x]", (int)grv_batch_hal_addr);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_GRV,grv_batch_data,grv_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
...
static void grv_poll_work_func(struct work_struct *work)
{
    ...
    polltime = atomic_read(&(grv_poll_info.poll_time));
    if (polltime > 0) {
        queue_delayed_work( (grv_poll_info.poll_wq),
                            &(grv_poll_info.poll_work),
                            msecs_to_jiffies(polltime) );
        SENSOR_A_LOG("start delay work :polltime[%d]",
                     atomic_read(&(grv_poll_info.poll_time)) );
    } else {
