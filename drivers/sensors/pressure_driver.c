...
static ssize_t pressure_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t pressure_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* pressure_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    pressure_batch_hal_addr = data_addr;

    SENSOR_N_LOG("pressure_batch_hal_addr[%x]", (int)pressure_batch_hal_addr);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_PRESSURE,pressure_batch_data,pressure_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
...
{
    SENSOR_N_LOG("start");
    atomic_set(&g_nCalP, offset);
    sensor_com_mutex_lock();
    sns_pressure_set_cal_ofs(offset);
...
static int32_t pressure_calibration_start(int32_t pres_mode, int32_t PressureBase)
{
    ...
    queue_delayed_work((pressure_calib_wq_info.poll_wq),
                       &(pressure_calib_wq_info.poll_work),
...
static void pressure_poll_work_func(struct work_struct *work)
{
    ...
    polltime = atomic_read(&(pressure_poll_info.poll_time));
    if (polltime > 0) {
        queue_delayed_work( (pressure_poll_info.poll_wq),
                            &(pressure_poll_info.poll_work),
                            msecs_to_jiffies(polltime) );
        SENSOR_A_LOG("start delay work :polltime[%d]",
                     atomic_read(&(pressure_poll_info.poll_time)) );
    } else {
...
void pressure_driver_init(void)
{
    ...
    SENSOR_N_LOG("sensor_input_init()-->ret[%d] dev[%d]",
                  ret, (int)(pressure_input_info.dev));
