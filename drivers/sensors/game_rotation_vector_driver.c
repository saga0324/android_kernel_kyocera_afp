...
static ssize_t game_rot_vctr_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t game_rot_vctr_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static DEVICE_ATTR(batch_data,
    ...
    game_rot_vctr_batch_data_store
);
...
static uint8_t* game_rot_vctr_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    game_rot_vctr_batch_hal_addr = data_addr;

    SENSOR_N_LOG("game_rot_vctr_batch_hal_addr[%x]", (int)game_rot_vctr_batch_hal_addr);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_GAME_ROT_VCTR,game_rot_vctr_batch_data,
                              game_rot_vctr_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
...
static void game_rot_vctr_poll_work_func(struct work_struct *work)
{
    ...
    polltime = atomic_read(&(game_rot_vctr_poll_info.poll_time));
    if (polltime > 0) {
        queue_delayed_work( (game_rot_vctr_poll_info.poll_wq),
                            &(game_rot_vctr_poll_info.poll_work),
                            msecs_to_jiffies(polltime) );
        SENSOR_A_LOG("start delay work :polltime[%d]",
                     atomic_read(&(game_rot_vctr_poll_info.poll_time)) );
    } else {
...
void game_rot_vctr_driver_init( void )
{
    ...
    SENSOR_N_LOG("sensor_input_init()-->ret[%d] dev[%d]",
                  ret, (int)(game_rot_vctr_input_info.dev) );
