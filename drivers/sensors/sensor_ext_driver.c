...
#define VIB_INTERLOCKING_OFF   0x00
    ...
    char *buf)
{
    ...
    if ( 0 == get_ret ) {
        ...
        if (0 == get_ret) {
            ret = snprintf( buf,  PAGE_SIZE, "%s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
                           buf,
...
static void sensor_ext_vib_interlocking_off(struct work_struct *work)
{
    ...
    sns_dailys_set_vib_interlocking(VIB_INTERLOCKING_OFF);
    ...
    size_t count)
{
    ...
    if(ope){
        if (SENSOR_ON == sensor_get_status(SENSOR_EXT_PEDO)) {
            ...
            sensor_com_mutex_lock();
            sns_dailys_set_vib_interlocking(VIB_INTERLOCKING_ON);
            sensor_com_mutex_unlock();
        }
    } else {
