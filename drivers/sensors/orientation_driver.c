...
static ssize_t ortn_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t ortn_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* ortn_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    ortn_batch_hal_addr = data_addr;

    SENSOR_N_LOG("ortn_batch_hal_addr[%x]", (int)ortn_batch_hal_addr);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_ORTN,ortn_batch_data,ortn_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
