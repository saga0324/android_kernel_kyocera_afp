...
static ssize_t mag_uncal_batch_store(struct device *dev,
    ...
    const char *buf,
    size_t count);
static ssize_t mag_uncal_batch_data_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* mag_uncal_batch_hal_addr = NULL;
    ...
    size_t count)
{
    ...
    mag_uncal_batch_hal_addr = data_addr;

    SENSOR_N_LOG("mag_uncal_batch_hal_addr[%x]", (int)mag_uncal_batch_hal_addr);

    if(sensor_copy_batch_uncal_data(SENSOR_BATCH_TBL_MAG_UNCAL, mag_uncal_batch_data,
                              mag_uncal2_batch_data, mag_uncal_batch_hal_addr)){
        mutex_unlock(&sensor_batch_mutex);
...
static void mag_uncal_poll_work_func(struct work_struct *work)
{
    ...
    polltime = atomic_read(&(mag_uncal_poll_info.poll_time));
    if (polltime > 0) {
        queue_delayed_work( (mag_uncal_poll_info.poll_wq),
                            &(mag_uncal_poll_info.poll_work),
                            msecs_to_jiffies(polltime) );
        SENSOR_A_LOG("start delay work :polltime[%d]",
                     atomic_read(&(mag_uncal_poll_info.poll_time)) );
    } else {
...
void mag_uncal_driver_init( void )
{
    ...
    SENSOR_N_LOG("sensor_input_init()-->ret[%d] dev[%d]",
                  ret, (int)(mag_uncal_input_info.dev) );
