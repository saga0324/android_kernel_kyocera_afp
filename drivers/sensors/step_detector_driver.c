...
static ssize_t step_dtc_batch_data_status_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
static ssize_t step_dtc_batch_data_1_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
static ssize_t step_dtc_batch_data_1_show(struct device *dev,
    struct device_attribute *attr,
    char *buf);
static ssize_t step_dtc_batch_data_2_store(struct device *dev,
    struct device_attribute *attr,
    const char *buf,
    size_t count);
...
static uint8_t* step_dtc_batch_hal_addr_1 = NULL;
static uint8_t* step_dtc_batch_hal_addr_2 = NULL;
    ...
    size_t count)
{
    ...
    step_dtc_batch_hal_addr_1 = data_addr;

    SENSOR_N_LOG("step_dtc_batch_hal_addr_1[%x]", (int)step_dtc_batch_hal_addr_1);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_REL_STEP_1,step_dtc_step1_batch_data,
                              step_dtc_batch_hal_addr_1)){
        step_dtc_step1_batch_data.payload_size = 0;
        step_dtc_step1_batch_data.recode_num = 0;
        mutex_unlock(&sensor_batch_mutex);
    ...
    size_t count)
{
    ...
    step_dtc_batch_hal_addr_2 = data_addr;

    SENSOR_N_LOG("step_dtc_batch_hal_addr_2[%x]", (int)step_dtc_batch_hal_addr_2);

    if(sensor_copy_batch_data(SENSOR_BATCH_TBL_REL_STEP_2,step_dtc_step2_batch_data,
                              step_dtc_batch_hal_addr_2)){
        mutex_unlock(&sensor_batch_mutex);
        SENSOR_ERR_LOG("miss copy batch data to user");
        return count;
    }
...
void step_dtc_driver_init(void)
{
    ...
    SENSOR_N_LOG("sensor_input_init()-->ret[%d] dev[%d]",
                  ret, (int)(step_dtc_input_info.dev) );
