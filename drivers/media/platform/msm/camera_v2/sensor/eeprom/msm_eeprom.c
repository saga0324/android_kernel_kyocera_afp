			      ...
			      struct msm_eeprom_memory_block_t *block)
{
	...
	rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
		&(e_ctrl->i2c_client), eeprom_read_settings[0].reg_addr,
		eeprom_read_settings[0].reg_data, 1);
	if (rc < 0) {
		pr_err("%s: eeprom read settings write failed. addr[%d]=0x%x, data[%d]=0x%x\n",
			__func__, j, eeprom_read_settings[j].reg_addr,j, eeprom_read_settings[j].reg_data);
		return rc;
	}

	msleep(5);

	for( j = 1; j < ARRAY_SIZE(eeprom_read_settings); j++)
	{
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
