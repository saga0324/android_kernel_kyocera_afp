	...
	enum msm_camera_i2c_data_type data_type)
{
	...
	rc = msm_camera_qup_i2c_rxdata(client, buf, client->addr_type, buf, data_type);
	if (rc < 0) {
		S_I2C_DBG("%s fail\n", __func__);
		kfree(buf);
		buf = NULL;
	...
	uint32_t addr, uint8_t *data, uint32_t num_byte)
{
	...
	if ((client->addr_type != MSM_CAMERA_I2C_BYTE_ADDR
		&& client->addr_type != MSM_CAMERA_I2C_WORD_ADDR
		&& client->addr_type != MSM_CAMERA_I2C_4B_ADDR)			/* for M12MO */
		|| num_byte == 0)
		return rc;

	if (client->addr_type == MSM_CAMERA_I2C_4B_ADDR) {	/* for M12MO */
		S_I2C_DBG("%s addr value is use for request length: addr= 0x%x\n",
			__func__, addr);
		req_byte = addr;
		buf = data;
		if (!buf){
			pr_err("%s:%d argument 'data' is NULL.\n", __func__, __LINE__);
			return -EINVAL;
		}
	} else {
		...
		req_byte = client->addr_type;
		buf = kzalloc(req_byte + num_byte, GFP_KERNEL);
		if (!buf){
			pr_err("%s:%d no memory.\n", __func__, __LINE__);
			return -ENOMEM;
		}
	...
	rc = msm_camera_qup_i2c_rxdata(client, buf, req_byte, data, num_byte);
	...
	uint32_t addr, uint8_t *data, uint32_t num_byte)
{
	...
	if (client->addr_type == MSM_CAMERA_I2C_4B_ADDR) {	/* for M12MO */
		S_I2C_DBG("%s addr value is no use: addr= 0x%x\n", __func__,
			addr);
		len = 0;
		buf = data;
		if (!buf){
	...
	struct msm_camera_i2c_seq_reg_setting *write_setting)
{
	...
	if ((write_setting->addr_type != MSM_CAMERA_I2C_BYTE_ADDR
		&& write_setting->addr_type != MSM_CAMERA_I2C_WORD_ADDR
		&& write_setting->addr_type != MSM_CAMERA_I2C_4B_ADDR)) {	/* for M12MO */
