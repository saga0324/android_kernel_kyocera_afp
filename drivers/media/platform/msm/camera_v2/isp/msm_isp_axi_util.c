...
void msm_isp_axi_stream_update(struct vfe_device *vfe_dev, uint8_t input_src)
{
	...
	for (i = 0; i < MAX_NUM_STREAM; i++) {
		if (axi_data->stream_info[i].state == START_PENDING ||
			...
			if ((1 <<
				...
				if (input_src &
					((1 << VFE_RAW_0) | (1 << VFE_RAW_1) | (1 << VFE_RAW_2))) {
	...
	if (vfe_dev->axi_data.stream_update == 0) {
		pr_info("%s call complete(). input_src=0x%02x\n",__func__, input_src);
	...
	enum msm_isp_camif_update_state camif_update)
{
	...
	pr_info("%s: call wait_for_completion_timeout(). timeout=%d, "
		"update stream/camif=%d/%d\n", __func__, VFE_MAX_CFG_TIMEOUT,
		vfe_dev->axi_data.stream_update, camif_update);
			...
			enum msm_isp_camif_update_state camif_update)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		if (src_state) {
			wait_for_complete = 1;
			pr_info("%s: wait_for_complete[%d]=%d: stream_src=%d, src_state=%d\n",
				__func__, i, wait_for_complete, stream_info->stream_src, src_state);
		} else {
			if (vfe_dev->dump_reg)
			...
			enum msm_isp_camif_update_state camif_update)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		if (!wait_for_complete_for_this_stream) {
			...
			pr_info("%s: wait_for_complete[%d]=%d: "
				"stream_src=%d, camif_update=%d, ext_read=%d, "
				"stream_type=%d, num_burst=%d, \n", __func__, i,
				wait_for_complete_for_this_stream,
				stream_info->stream_src, camif_update, ext_read,
