...
void msm_gpio_dbg_print(void) {
	...
	pctrl = container_of(g_msm_gpio_chip, struct msm_pinctrl, chip);
	...
	for (offset = 0; offset < g_msm_gpio_chip->ngpio; offset++) {
		...
		if(!is_out) {
			pr_notice("GPIO[%d.]: [FS]0x%x, [DIR]IN, [PULL]%s, [DRV]%dmA\n",
						offset, func, pulls[pull], msm_regval_to_drive(drive));
		}else{
			val = readl(pctrl->regs + g->io_reg);
			pr_notice("GPIO[%d.]: [FS]0x%x, [DIR]OUT, [PULL]%s, [DRV]%dmA, [VAL]%s\n",
						offset, func, pulls[pull], msm_regval_to_drive(drive), !!(val & BIT(g->in_bit)) ? "HIGH" : "LOW");
		}
	}
