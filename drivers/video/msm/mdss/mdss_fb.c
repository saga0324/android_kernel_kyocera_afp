...
int mdss_fb_blank_sub(int blank_mode, struct fb_info *info,
			     int op_enable);
...
static DEVICE_ATTR(kc_test_mode, S_IRUGO | S_IWUSR,
	kdisp_panel_get_test_mode, kdisp_panel_set_test_mode);
static DEVICE_ATTR(kctrl_info, S_IRUGO | S_IWUSR,
	kdisp_panel_get_kctrl_info, NULL);
...
static struct attribute *mdss_fb_attrs[] = {
	...
	&dev_attr_kc_test_mode.attr,
	&dev_attr_kctrl_info.attr,
	NULL,
};
...
extern int disp_ext_panel_power_onoff(struct mdss_panel_data *pdata, int enable);
...
static void mdss_fb_shutdown(struct platform_device *pdev)
{
	...
	ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	kdisp_bl_ctrl_onoff(&ctrl->kc_ctrl_data, 0);
	...
	kdisp_mipiic_panel_power_ctrl(&ctrl->kc_ctrl_data, 0);
	pr_debug("%s: End\n",__func__);
}
			 ...
			 unsigned long arg, struct file *file)
{
	...
	if (kdisp_com_is_invalid_all()) {
		pr_err("[DISP] skip %s: KFBM and Panel Not Connect\n", __func__);
		return 0;
	}
	...
	case MSMFB_DISPLAY_COMMIT:
		if (kdisp_com_is_invalid() ||
		   (kdisp_connect_get_panel_detect() == PANEL_NOT_FOUND) ) {
			pr_info("%s: Panel not connected or Invalid\n", __func__);
			mdss_fb_release_fences(mfd);
			ret = 0;
			goto exit;
		}
	...
	case KDISP_SUSPEND_SW_REFRESHER:
		ret = kdisp_diag_ioctl_handler(info, cmd, argp);
		break;
