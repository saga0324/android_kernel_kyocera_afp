...
static DEVICE_ATTR(kc_test_mode, S_IRUGO | S_IWUSR,
	disp_ext_panel_get_test_mode, disp_ext_panel_set_test_mode);
...
static void mdss_fb_shutdown(struct platform_device *pdev)
{
	...
	light_led_disp_power_set(LIGHT_MAIN_WLED_LCD_PWRDIS);
			 ...
			 unsigned long arg)
{
	...
	if (disp_ext_is_invalid_all()) {
		pr_err("[Display] skip %s: KFBM and Panel Not Connect\n", __func__);
		return 0;
	}
