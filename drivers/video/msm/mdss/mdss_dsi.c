...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	disp_ext_debug_dump_dsi_timing_reg(ctrl_pdata->phy_io.base);
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if((gpio_get_value(ctrl_pdata->rst_gpio) == 0) && (disp_ext_board_get_panel_detect() == 1))
	{
		ctrl_pdata->panel_data.panel_info.cont_splash_enabled = 0;
		pr_debug("%s: set splash disable\n", __func__);
	}
