...
extern int mdss_dsi_panel_off_post(struct mdss_panel_data *pdata);
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if (pdata->panel_info.cont_splash_enabled ||
		...
		if (pdata->panel_info.cont_splash_enabled) {
			disp_ext_panel_power_onoff(pdata, 1);
		}
...
static int mdss_dsi_blank_post(struct mdss_panel_data *pdata)
{
	int rc = 0;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	if (pdata->panel_info.type == MIPI_VIDEO_PANEL &&
			ctrl_pdata->off_post_cmds.link_state == DSI_LP_MODE) {
		mdss_dsi_sw_reset(ctrl_pdata, false);
		mdss_dsi_host_init(pdata);
	}
