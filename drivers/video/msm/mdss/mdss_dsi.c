...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	kdisp_debug_dump_dsi_timing_reg(ctrl_pdata->phy_io.base);
end:
	pr_debug("%s-:\n", __func__);
	return ret;
}
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_BLANK:
		...
		kdisp_bl_ctrl_onoff(&ctrl_pdata->kc_ctrl_data, 0);
	...
	case MDSS_EVENT_PANEL_CLK_CTRL:
		pr_debug("%s: MDSS_EVENT_PANEL_CLK_CTRL\n", __func__);
	...
	case MDSS_EVENT_FB_REGISTERED:
		...
		break;
	default:
		if (!kdisp_dsi_event_handler(pdata, event))
			break;
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	kdisp_com_init(pdev, dsi_pan_node, &ctrl_pdata->kc_ctrl_data); //chuui
	...
	struct device_node *pan_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if((gpio_get_value(ctrl_pdata->rst_gpio) == 0) && (kdisp_connect_get_panel_detect() == 1))
	{
		ctrl_pdata->panel_data.panel_info.cont_splash_enabled = 0;
		pr_notice("%s: set splash disable\n", __func__);
	}

	if (pinfo->cont_splash_enabled) {
		rc = kdisp_mipiic_panel_power_ctrl(&ctrl_pdata->kc_ctrl_data, 1);
		if (rc < 0) {
			pr_err("%s:%d, kdisp_mipiic_panel_power_ctrl fail mipi_vdd\n",__func__, __LINE__);
			return rc;
		}
	}
