				...
				struct mdp3_dma *dma)
{
	...
	if(panel_info->bpp == 16)
	{
		outputConfig.dither_en = 1;
		outputConfig.color_comp_out_bits = (MDP3_DMA_OUTPUT_COMP_BITS_6 << 4) |
						 (MDP3_DMA_OUTPUT_COMP_BITS_6 << 2)| MDP3_DMA_OUTPUT_COMP_BITS_6;
	}
	else
	{
		outputConfig.dither_en = 0;
		outputConfig.color_comp_out_bits = (MDP3_DMA_OUTPUT_COMP_BITS_8 << 4) |
						(MDP3_DMA_OUTPUT_COMP_BITS_8 << 2)| MDP3_DMA_OUTPUT_COMP_BITS_8;
	}
...
//	pr_debug("%s: index=%d, data.len=%d, stride=%d, height=%ld\n",__func__,
					...
					struct mdp_display_commit *cmt_data)
{
	...
	if (mdp3_session->first_commit) {
		if (!mdp3_session->in_splash_screen)
			panel->event_handler(panel, KDISP_EVENT_PANEL_ON_POST1, NULL);
	}
...
frame_done:
	...
	if (mdp3_session->first_commit) {
		...
		if (!mdp3_session->in_splash_screen)
			panel->event_handler(panel, KDISP_EVENT_PANEL_ON_POST2, NULL);
		mdp3_session->first_commit = false;

		kdisp_bl_ctrl_onoff(&ctrl_pdata->kc_ctrl_data, 1);
		if (panel)
			rc |= panel->event_handler(panel,
...
static void mdp3_ctrl_pan_display(struct msm_fb_data_type *mfd)
{
	...
	if (mdp3_session->first_commit) {
		...
		if (panel_info->mipi.post_init_delay)
			usleep_range(((1000 / panel_info->mipi.frame_rate) + 1) *
					panel_info->mipi.post_init_delay,((1000 / panel_info->mipi.frame_rate) + 1) *
					panel_info->mipi.post_init_delay);
		else
			usleep_range(1000 / panel_info->mipi.frame_rate,1000 / panel_info->mipi.frame_rate);
