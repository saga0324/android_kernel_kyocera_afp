...
static int32_t dtv_on_counter = 0;
...
)
{
	...
	if(dtv_on_counter == 0){
		pr_info("dtvtuner_pm_dev_tuner_power_on: qlpower_spi_request call\n");
		qlpower_spi_request(D_DTVTUNER_PM_DRIVER_NAME);
		dtv_on_counter++;
	}
	else{
		pr_err("dtvtuner_pm_dev_tuner_power_on: dtv on counter invalid!\n");
	}
...
)
{
	...
	if(dtv_on_counter == 1){
		pr_info("dtvtuner_pm_dev_tuner_power_off: qlpower_spi_release call\n");
		qlpower_spi_release(D_DTVTUNER_PM_DRIVER_NAME);
		dtv_on_counter = 0;
	}
	else{
		pr_err("dtvtuner_pm_dev_tuner_power_off: dtv on counter invalid!\n");
	}
	return 0;
