...
static struct wake_lock pi_usb_wake_lock_attach;
...
{
	wake_lock(&pi_usb_wake_lock_attach);
	...
	if (pi_usb->attach_state) {
		...
		if(STS_MODE_AUDIO == (pi_usb->reg_data.port_status & STS_MODE_MASK)) {
			...
			swic_notify_from_cclogic(SWIC_MODE_PLUGOUT_TO_AUDIO);
			...
			swic_notify_from_cclogic(SWIC_MODE_PLUGOUT_TO_USB);
		}
    } else if(!pi_usb->attach_state && pi_usb->b_is_audio) {
		...
		swic_notify_from_cclogic(SWIC_MODE_AUDIO_TO_PLUGOUT);
	} else if(!pi_usb->attach_state && !pi_usb->b_is_audio) {
		/* Notify to SWIC of Exit USB */
		swic_notify_from_cclogic(SWIC_MODE_USB_TO_PLUGOUT);
	}

	dev_dbg(&pi_usb->client->dev, "%s end \n", __func__);
	wake_unlock(&pi_usb_wake_lock_attach);
}
...
static int piusb_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	wake_lock_init( &pi_usb_wake_lock_attach, WAKE_LOCK_SUSPEND, "pi_usb_attach" );
...
static int piusb_remove(struct i2c_client *i2c)
{
	...
	wake_lock_destroy( &pi_usb_wake_lock_attach );
