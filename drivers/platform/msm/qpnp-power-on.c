...
static u32 g_resin_state_irq = 0;
static struct input_dev *g_resin_pon_input = NULL;
static u32 g_resin_key_code = 0;
void qpnp_enable_key_lock_mode(void)
{
	if(g_resin_state_irq != 0)
	{
		disable_irq_wake(g_resin_state_irq);
	}
	if(g_resin_pon_input != NULL && g_resin_key_code != 0) {
		input_report_key(g_resin_pon_input, g_resin_key_code, 0);
		input_sync(g_resin_pon_input);
	}
}
void qpnp_disable_key_lock_mode(void)
{
	if(g_resin_state_irq != 0)
	{
		enable_irq_wake(g_resin_state_irq);
	}
}
...
static irqreturn_t qpnp_resin_irq(int irq, void *_pon)
{
	...
	if(!is_key_lock_mode()) {
		...
		pr_debug("%s : skip by key_lock_mode_on\n",__func__);
	}
	return IRQ_HANDLED;
}
...
qpnp_pon_request_irqs(struct qpnp_pon *pon, struct qpnp_pon_config *cfg)
{
	...
	case PON_RESIN:
		...
		g_resin_state_irq = cfg->state_irq;
		g_resin_pon_input = pon->pon_input;
		g_resin_key_code = cfg->key_code;
		if (cfg->use_bark) {
			rc = devm_request_irq(&pon->spmi->dev, cfg->bark_irq,
						qpnp_resin_bark_irq,
						IRQF_TRIGGER_RISING,
