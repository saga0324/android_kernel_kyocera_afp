 ...
 * (C) 2016-2017 KYOCERA Corporation
 */
...
static void qpnp_vib_send_uevent(struct device *dev, int on)
{
...
#ifdef SENSOR_VIB_INTERLOCKING
	if(vib_lasttime_val != on) {
		qpnp_vib_send_uevent(vib->timed_dev.dev, on);
		vib_lasttime_val = on;
	}
#endif
