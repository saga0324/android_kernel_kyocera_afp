...
extern void detect_ecc_error(unsigned int err_addr, unsigned int device_status);
...
static int card_busy_detect(struct mmc_card *card, unsigned int timeout_ms,
		bool hw_busy_detect, struct request *req, int *gen_err, bool from_stop_cmd)
{
	...
	do {
		...
		if ((R1_CURRENT_STATE(status) == R1_STATE_RCV) &&
		  (status & R1_WP_VIOLATION) && !(from_stop_cmd)) {
			pr_notice("%s: R1_WP_VIOLATION has occurred, eMMC current status (0x%x)\n",__func__, status);

			err = send_stop(card, 0, req, gen_err, &stop_status);
			if (err)
		...
		struct request *req, int *gen_err, u32 *stop_status)
{
	...
	*stop_status = cmd.resp[0];
	...
	return card_busy_detect(card, timeout_ms, use_r1b_resp, req, gen_err, true);
}
			     ...
			     struct mmc_async_req *areq)
{
	...
	if (!mmc_host_is_spi(card->host) && rq_data_dir(req) == READ) {
		...
		u32 stop_status2;
		int gen_err;
	    u32 cmd_arg = brq->cmd.arg;
		int err2 = get_card_status(card, &status2, 0);
		if (err2) {
			pr_err("%s: error %d read requesting status\n",
			       req->rq_disk->disk_name, err2);
		}
	    else if (R1_CURRENT_STATE(status2) == R1_CARD_ECC_FAILED) {
			pr_err("%s: 21bit ECC ERROR, eMMC current status (0x%x)\n",__func__, status2);

			err2 = send_stop(card, 0, req, &gen_err, &stop_status2);
//			err2 = send_stop(card, &stop_status2);
			if (err2)
				pr_err("%s: read general error sending stop command\n",__func__);

		    detect_ecc_error(cmd_arg, status2);
		}
	}
