...
{
	int ret;
	ret = mmc_send_cxd_data(card, card->host, MMC_SEND_EXT_CSD,
			ext_csd, 512);
	if(!ret) {
		if(card->host->index == 0 && card->cid.manfid == 0x90) {
			ext_csd[EXT_CSD_PART_SWITCH_TIME]  = 0x10;
			ext_csd[216]                       = 0x0E;
			ext_csd[EXT_CSD_GENERIC_CMD6_TIME] = 0x10;
		}
	}
	return ret;
}
