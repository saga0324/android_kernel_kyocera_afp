...
{
	{ 0,0,3,3,0,3,3,3,3,3,3,3,3,3,3,3,3,3, },
	{ 0,0,0,0,0,3,3,3,3,3,3,3,3,3,3,3,3,3, },
	{ 0,0,0,0,0,3,3,3,3,3,3,3,3,3,3,3,3,3, },
	{ 3,3,3,3,3,1,3,2,3,3,3,3,3,3,3,3,3,3, },
	{ 3,3,3,3,3,3,2,2,3,3,3,3,3,3,3,3,3,3, },
	{ 3,3,0,0,0,1,2,3,3,3,3,3,3,3,3,3,3,3, },
...
static int key_lock_mode_on = 0;
int is_key_lock_mode(void)
{
	return key_lock_mode_on;
}
static int oem_keylock_ctrl(const char *val, struct kernel_param *kp)
{
	int ret;
	...
	pr_err("key_lock_mode_on = %d\n", key_lock_mode_on);

	if(is_key_lock_mode()) {
		qpnp_enable_key_lock_mode();
	}else{
		qpnp_disable_key_lock_mode();
	}

	return 0;
}
module_param_call(key_lock_mode_on, oem_keylock_ctrl, param_get_int, &key_lock_mode_on, 0664);
MODULE_PARM_DESC(key_lock_mode_on, "key key_lock_mode_on");
...
#define MAX_ENABLE_CLEAR_WORK 32
static int g_ignore_clear_count = 0;
static int g_enable_clear_work_idx = 0;
static spinlock_t ignore_clear_lock;
static unsigned long ignore_clear_lock_flags;
static struct delayed_work enable_clear_work[MAX_ENABLE_CLEAR_WORK];

#define MAX_REPORT_CLEAR_WORK 32
static bool g_ignore_report_clear = false;
static int g_report_clear_count = 0;
static int g_report_clear_work_idx = 0;
static spinlock_t report_clear_work_lock;
static unsigned long report_clear_work_lock_flags;
static struct delayed_work report_clear_work[MAX_REPORT_CLEAR_WORK];

struct report_clear_info {
	struct input_dev *dev;
	unsigned int code;
	int value;
	struct list_head list;
};
struct list_head report_clear_info_head;
							...
							bool active)
{
	...
	if (active) {
		if(OEM_get_board() == OEM_BOARD3_TYPE) {
			set_state =
				pinctrl_lookup_state(keypad->matrix_keypad_pinctrl,
							"matrix_keypad_active_hw03");
		}else{
			set_state =
				pinctrl_lookup_state(keypad->matrix_keypad_pinctrl,
...
void enable_clear_func(struct work_struct *work)
{
	spin_lock_irqsave(&ignore_clear_lock, ignore_clear_lock_flags);
	g_ignore_clear_count--;
	spin_unlock_irqrestore(&ignore_clear_lock, ignore_clear_lock_flags);
}

void report_clear_func(struct work_struct *work)
{
	struct report_clear_info *report_clear_info_ptr;

	spin_lock_irqsave(&report_clear_work_lock, report_clear_work_lock_flags);
	report_clear_info_ptr = list_entry(report_clear_info_head.next, struct report_clear_info, list);
	if(!(g_ignore_report_clear == true && report_clear_info_ptr->value)) {
		input_report_key(report_clear_info_ptr->dev,
						 report_clear_info_ptr->code,
						 report_clear_info_ptr->value);
		input_sync(report_clear_info_ptr->dev);
	}else{
		MATRIX_KEYPAD_DEBUG_LOG_PRINT("%s: skip report clear key\n",__func__);
	}
	list_del(report_clear_info_head.next);
	kfree(report_clear_info_ptr);
	g_report_clear_count--;
	spin_unlock_irqrestore(&report_clear_work_lock, report_clear_work_lock_flags);
}
...
static void matrix_keypad_scan(struct work_struct *work)
{
	...
	for (col = 0; col < pdata->num_col_gpios; col++) {
		...
		for (row = 0; row < pdata->num_row_gpios; row++) {
			...
			if(keycodes[code] == KEY_CLEAR && (new_state[col] & (1 << row))) {
				if(g_ignore_clear_count > 0) {
					MATRIX_KEYPAD_DEBUG_LOG_PRINT("%s: skip report clear key\n",__func__);
					continue;
				}
			}
			if(keycodes[code] == KEY_DOWN && (new_state[col] & (1 << row))) {
				spin_lock_irqsave(&report_clear_work_lock, report_clear_work_lock_flags);
				g_ignore_report_clear = true;
				spin_unlock_irqrestore(&report_clear_work_lock, report_clear_work_lock_flags);
				for(rcw_idx = 0; rcw_idx < MAX_REPORT_CLEAR_WORK; rcw_idx++) {
					flush_delayed_work(&report_clear_work[rcw_idx]);
				}
				spin_lock_irqsave(&report_clear_work_lock, report_clear_work_lock_flags);
				g_ignore_report_clear = false;
				spin_unlock_irqrestore(&report_clear_work_lock, report_clear_work_lock_flags);

				spin_lock_irqsave(&ignore_clear_lock, ignore_clear_lock_flags);
				if(g_ignore_clear_count < MAX_ENABLE_CLEAR_WORK) {
					g_ignore_clear_count++;
					schedule_delayed_work(&enable_clear_work[g_enable_clear_work_idx], msecs_to_jiffies(70));
					g_enable_clear_work_idx++;
					if(g_enable_clear_work_idx == MAX_ENABLE_CLEAR_WORK) {
						g_enable_clear_work_idx = 0;
					}
				}
				spin_unlock_irqrestore(&ignore_clear_lock, ignore_clear_lock_flags);
			}
            ...
            wake_lock_timeout(&matrix_keypad_wake_lock,HZ);
				...
				if( key_point && ( keycodes[code] == 0x67 || keycodes[code] == 0x6c ||
					...
					fake_touch = false;
					if(keycodes[code] == KEY_CLEAR) {
						spin_lock_irqsave(&report_clear_work_lock, report_clear_work_lock_flags);
						if(g_report_clear_count < MAX_REPORT_CLEAR_WORK) {
							report_clear_info_ptr = kmalloc(sizeof(struct report_clear_info), GFP_KERNEL);
							if(report_clear_info_ptr == NULL) {
								pr_err("%s: Failed kmalloc for report_clear_info_ptr\n",__func__);
								spin_unlock_irqrestore(&report_clear_work_lock, report_clear_work_lock_flags);
								continue;
							}
							report_clear_info_ptr->dev = input_dev;
							report_clear_info_ptr->code = keycodes[code];
							report_clear_info_ptr->value = new_state[col] & (1 << row);
							list_add_tail(&report_clear_info_ptr->list, &report_clear_info_head);
							g_report_clear_count++;
							schedule_delayed_work(&report_clear_work[g_report_clear_work_idx], msecs_to_jiffies(70));
							g_report_clear_work_idx++;
							if(g_report_clear_work_idx == MAX_REPORT_CLEAR_WORK) {
								g_report_clear_work_idx = 0;
							}
						}
						spin_unlock_irqrestore(&report_clear_work_lock, report_clear_work_lock_flags);
					}else{
...
#endif
	if(is_key_lock_mode()) {
		MATRIX_KEYPAD_DEBUG_LOG_PRINT("skip by key_lock_mode_on\n");
		goto out;
	}
...
static int matrix_keypad_resume(struct device *dev)
{
	...
	if(g_ignore_clear_count != 0) {
		pr_err("%s : g_ignore_clear_count is %d\n", __func__, g_ignore_clear_count);
		for(ecw_idx = 0; ecw_idx < MAX_ENABLE_CLEAR_WORK; ecw_idx++)
		{
			flush_delayed_work(&enable_clear_work[ecw_idx]);
		}
		g_ignore_clear_count = 0;
	}

	if(g_report_clear_count != 0) {
		pr_err("%s : g_report_clear_count is %d\n", __func__, g_report_clear_count);
		for(rcw_idx = 0; rcw_idx < MAX_REPORT_CLEAR_WORK; rcw_idx++)
		{
			flush_delayed_work(&report_clear_work[rcw_idx]);
		}
		while(!list_empty(&report_clear_info_head))
		{
			report_clear_info_ptr = list_entry(report_clear_info_head.next, struct report_clear_info, list);
			input_report_key(report_clear_info_ptr->dev,
							 report_clear_info_ptr->code,
							 report_clear_info_ptr->value);
			input_sync(report_clear_info_ptr->dev);
			list_del(&report_clear_info_ptr->list);
			kfree(report_clear_info_ptr);
		}
		g_report_clear_count = 0;
	}
...
static int matrix_keypad_probe(struct platform_device *pdev)
{
	...
	keypad->stopped = true;
	spin_lock_init(&ignore_clear_lock);
	spin_lock_init(&report_clear_work_lock);
	INIT_LIST_HEAD(&report_clear_info_head);
	for(ecw_idx = 0; ecw_idx < MAX_ENABLE_CLEAR_WORK; ecw_idx++) {
		INIT_DELAYED_WORK(&enable_clear_work[ecw_idx], enable_clear_func);
	}
	for(rcw_idx = 0; rcw_idx < MAX_REPORT_CLEAR_WORK; rcw_idx++) {
		INIT_DELAYED_WORK(&report_clear_work[rcw_idx], report_clear_func);
	}
