...
static int matrix_keypad_key_group[MATRIX_MAX_ROWS][MATRIX_MAX_COLS] =
{
	...
	{ 3,3,0,0,0,1,3,3,3,3,3,3,3,3,3,3,3,3, },
			   ...
			   int col, bool on)
{
	...
	if (on) {
		...
		gpio_direction_output(pdata->col_gpios[col], !level_on);
    	if (pdata->col_scan_high_delay_us)
    		udelay(pdata->col_scan_high_delay_us);
#endif
		gpio_direction_input(pdata->col_gpios[col]);
	}
}
...
static void matrix_keypad_scan(struct work_struct *work)
{
	...
	for (col = 0; col < pdata->num_col_gpios; col++) {
		...
		for (row = 0; row < pdata->num_row_gpios; row++)
			...
			if( row_asserted(pdata, row) ){
				...
				if ( (keycodes[code] == KEY_VOLUMEDOWN) ||
					 (keycodes[code] == KEY_F5) ||
					 (keycodes[code] == KEY_F6) ) {
					count_outside_folder++;
				}
...
#ifndef QUALCOMM_ORIGINAL_FEATURE
	if( (count != 0) && !(keypad->stopped) && ((folder_state == false) || (count_outside_folder != 0)))
	...
	if(fake_touch){
		...
		if(!last_key_code){
			...
			if(keypad->touch_event_wq)
				queue_delayed_work(keypad->touch_event_wq, &keypad->touch_event_work, msecs_to_jiffies(100));
			mutex_unlock(&keypad->input_report_lock);
		}
	} else {
		if (!g_kdm_matrixkey_check) {
...
#ifndef QUALCOMM_ORIGINAL_FEATURE
	if(count==0 || keypad->stopped || ((folder_state == true) && (count_outside_folder == 0))){
#endif /* Not QUALCOMM_ORIGINAL_FEATURE */
	keypad->scan_pending = false;
	enable_row_irqs(keypad);
	wake_unlock(&matrix_keypad_scan_wake_lock);
    MATRIX_KEYPAD_PR_LOG_PRINT("end cnt = %d stop = %d fd = %d \n", count, keypad->stopped, folder_state);
#ifndef QUALCOMM_ORIGINAL_FEATURE
	}else {
...
#ifndef QUALCOMM_ORIGINAL_FEATURE
	of_property_read_u32(np, "col-scan-high-delay-us",
						&pdata->col_scan_high_delay_us);
#endif /* Not QUALCOMM_ORIGINAL_FEATURE */
