...
#include "uapi/linux/kaudio_api.h"

#undef DEBUG_KEYLOG_ENABLE
#ifdef DEBUG_KEYLOG_ENABLE
#define KEY_LOG_PRINT(fmt, ...) printk(KERN_ERR fmt, ##__VA_ARGS__)
#else
#define KEY_LOG_PRINT(fmt, ...)
#endif
#define KEY_LOG_PRINT_SEND(fmt, ...) printk(KERN_DEBUG fmt, ##__VA_ARGS__)
...
static struct gpio_button_data *gpio_power_sw_data;
static bool gpio_keys_in_folder_wakeup_irq_set;
...
EXPORT_SYMBOL(gpio_keys_is_stateon);

bool gpio_keys_power_sw_stateon(void)
{
	bool state;

	if(gpio_get_value(gpio_power_sw_data->button->gpio) == 0)
		state = true;
	else
		state = false;
...
EXPORT_SYMBOL(gpio_keys_power_sw_stateon);
	...
	bool is_on;
		...
		if (state) {
			...
			if (button->on_cnt) {
				...
				if ((gpio_keys_is_stateon(SW_LID) == false) ||
				    ((button->code != KEY_F8) &&
				     (button->code != KEY_F9) &&
				     (button->code != KEY_F10))) {
			    if (g_kdm_check) {
                    ...
                    if(SW_LID == button->code){
                        key_debug_marker("folder close report", !!state);
                    }
					else{
			...
			if (g_kdm_check) {
                KEY_LOG_PRINT("release not key_set_code \n");
                    ...
                    if(SW_LID == button->code){
                        key_debug_marker("folder open report", !!state);
                    }
					else{
...
#ifndef QUALCOMM_ORIGINAL_FEATURE
	if (bdata->button->code == SW_LID) {
		if (state) {
			kaudio_codec_sidetone_mute(KAUDIO_SIDETONE_MUTE);
		} else {
			kaudio_codec_sidetone_mute(KAUDIO_SIDETONE_UNMUTE);
		}
	}
#endif
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		if (of_property_read_u32(pp, "on_chattering_num", &reg) == 0){
			button->on_chattering_num = reg;
			button->on_cnt = reg;
        }
		else{
		...
		if (of_property_read_u32(pp, "off_chattering_num", &reg) == 0){
			button->off_chattering_num = reg;
			button->off_cnt = reg;
        }
		else{
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	#ifndef QUALCOMM_ORIGINAL_FEATURE
		if (bdata->button->code == SW_POWER) {
			gpio_power_sw_data = &ddata->data[i];
		}
	#endif
...
static int gpio_keys_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		for (i = 0; i < ddata->pdata->nbuttons; i++) {
			struct gpio_button_data *bdata = &ddata->data[i];
#ifndef QUALCOMM_ORIGINAL_FEATURE
			if ((folder_state == false) || 
			    ((bdata->button->code != KEY_F8) &&
			     (bdata->button->code != KEY_F9) &&
			     (bdata->button->code != KEY_F10))) {
#endif /* Not QUALCOMM_ORIGINAL_FEATURE */
			if (bdata->button->wakeup)
...
#ifndef QUALCOMM_ORIGINAL_FEATURE
		if (folder_state == false)
			gpio_keys_in_folder_wakeup_irq_set = true;
		else
			gpio_keys_in_folder_wakeup_irq_set = false;
#endif /* Not QUALCOMM_ORIGINAL_FEATURE */
	} else {
...
static int gpio_keys_resume(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		for (i = 0; i < ddata->pdata->nbuttons; i++) {
			struct gpio_button_data *bdata = &ddata->data[i];
#ifndef QUALCOMM_ORIGINAL_FEATURE
			if ((gpio_keys_in_folder_wakeup_irq_set == true) ||
			    ((bdata->button->code != KEY_F8) &&
			     (bdata->button->code != KEY_F9) &&
			     (bdata->button->code != KEY_F10))) {
#endif /* Not QUALCOMM_ORIGINAL_FEATURE */
