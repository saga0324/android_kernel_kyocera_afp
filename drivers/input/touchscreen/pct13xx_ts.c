...
#define MODULE_MOD_TIME		"20160509"
#define DRIVER_VERSION		"PixArt PCT13XX - v2.2"
...
static int pct1332qn_mass_erase_flash(void)
{
	uint8_t val, retry;

	if(pct13xx_switch_bank(1) < 0){
		pr_info(MODULE_NAME "%s: mass erase flash fail due to switch bank 1\n", __func__);
		return -EIO;
	}

	if(i2c_write(PCT13XX_REG_USER_MODE, PCT13XX_USER_CMD) < 0){
		pr_info(MODULE_NAME "%s: mass erase flash fail due to write 0x5e\n", __func__);
		return -EIO;
	}

	if(i2c_write(PCT13XX_REG_FLASH_EXT_HI, 0x00) < 0){
		pr_info(MODULE_NAME "%s: mass erase flash fail due to write 0x60\n", __func__);
		return -EIO;
	}

	if(i2c_write(PCT13XX_REG_FLASH_EXT_LO, 0x00) < 0){
		pr_info(MODULE_NAME "%s: mass erase flash fail due to write 0x61\n", __func__);
		return -EIO;
	}
	...
	do {
		if(i2c_write(PCT13XX_REG_IFREN, PCT13xx_IB_DISABLE) < 0){
			pr_info(MODULE_NAME "%s: mass erase flash fail due to write 0x6e\n", __func__);
			return -EIO;
		}

		if(i2c_read(PCT13XX_REG_IFREN, &val, 1) < 0){
			pr_info(MODULE_NAME "%s: mass erase flash fail due to read 0x6e\n", __func__);
			return -EIO;
		}

		if (val == PCT13xx_IB_DISABLE)
			break;
		msleep(10);
	} while (--retry > 0);
	if (!retry) {
		pr_err(MODULE_NAME "%s mass erase flash fail due to IB not 0\n", __func__);
		return -EBUSY;
	}

	if(i2c_write(PCT13XX_REG_FLASH_CMD, PCT13XX_FLASH_MASS_ERASE) < 0){
		pr_info(MODULE_NAME "%s: mass erase flash fail due to write 0x62\n", __func__);
		return -EIO;
	}

	if(i2c_write(PCT13XX_REG_START, PCT13XX_START) < 0){
		pr_info(MODULE_NAME "%s: mass erase flash fail due to write 0x5f\n", __func__);
		return -EIO;
	}

	retry = 20;
	do {
		if(i2c_read(PCT13XX_REG_FLASH_BUSY, &val, 1) < 0){
			pr_info(MODULE_NAME "%s: mass erase flash fail due to read 0x70\n", __func__);
			return -EIO;
		}

		if (val == PCT13XX_FLASH_IDLE)
			break;
		msleep(10);
	} while (--retry > 0);
	if (!retry) {
		pr_err(MODULE_NAME "%s mass erase flash fail due to flash busy\n", __func__);
		return -EBUSY;
	}
...
static int pct13xx_resume_from_sram(void)
{
	if(pct13xx_switch_bank(6) < 0){
		pr_info(MODULE_NAME "%s: resume from sram fail due to switch bank 6\n", __func__);
		return -EIO;
	}

	if(i2c_write(PCT13XX_REG_SHUTDOWN, PCT13XX_SHUTDOWN) < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to write 0x7D\n", __func__);
		return -EIO;
	}

	if(i2c_write(PCT13XX_REG_SCRAM_BOOTLOAD, 0x01) < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to write 0x7D\n", __func__);
		return -EIO;
	}

	if(i2c_write(PCT13XX_REG_SHUTDOWN, PCT13XX_HARD_RESET) < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to write 0x7D\n", __func__);
		return -EIO;
	}
...
static int pct1332qn_write_flash(struct pct13xx_data *pd, int address, uint8_t *data)
{
	int len = sizeof(fw_pct1332qn);
	int pageSize = 1024, remainedSize;
	uint8_t pageData[pageSize];
	uint8_t CPUStopFW[6] = {0x90, 0xd1, 0x14, 0x74, 0x02, 0xf0};
	...
	if(pct13xx_switch_bank(1) < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to switch bank 1\n", __func__);
		return -EIO;
	}
	...
	if(pct1332qn_write_sram(CPUStopFW, 0, sizeof(CPUStopFW)) < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to write CPUSTopFW to sram\n", __func__);
		return -EIO;
	}
	if(pct13xx_resume_from_sram() < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to resume from sram\n", __func__);
		return -EIO;
	}

	udelay(1500);

	if(i2c_write(PCT13XX_REG_SCRAM_BOOTLOAD, 0x00) < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to write 0x7D\n", __func__);
		return -EIO;
	}

	if(pct1332qn_mass_erase_flash() < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to pct1332qn_mass_erase_flash\n", __func__);
		return -EIO;
	}

	if(pct1332qn_write_flash_page(address, CPUStopFW, sizeof(CPUStopFW)) < 0)
	{
		pr_info(MODULE_NAME "%s: write flash fail due to write CPUStopFW\n", __func__);
		return -EIO;
	}
	...
	memcpy(pageData, data + (address << 10), 1024);
	if(pct1332qn_write_flash_page(address, pageData, 1024) < 0){
		pr_info(MODULE_NAME "%s: write flash fail due to write flash page(1)\n", __func__);
		return -EIO;
	}
...
static int pct1332qn_update_firmware(struct pct13xx_data *pd, uint8_t *flashcode)
{
	uint8_t i, retry = 5;
	...
	for(i = 0; i < retry; i++){
		if(pct1332qn_write_flash(pd, 0, flashcode) < 0 ){
			pr_info(MODULE_NAME "%s: retry i is: %d, update firmware fail due to write flash\n", __func__, i);
		}
		else{
			break;
		}
	...
	if(i == 5){
		pr_info(MODULE_NAME "%s: retry reach boundary, update firmware failed\n", __func__);
		return -EIO;
	}
...
static int pct1332qn_load_fw(struct pct13xx_data *pd, uint8_t force_download)
{
	...
	pct1332qn_update_firmware(pd, pfw);
	/*check crc*/
	if(pct1332qn_calc_crc(pd, crc_result, PCT13XX_CRC_FW) < 0 ){
