...
static int cyttsp5_normal_and_resume(struct cyttsp5_core_data *cd)
{
	...
	rc = regulator_set_optimum_mode(cd->sensor_vdd, 15000);
	if(rc < 0){
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->sensor_vdd = regulator_get(dev, "vdd");
	if(IS_ERR(cd->sensor_vdd)){
		pr_err("%s: vdd: regulator_get fail.\n", __func__);
		goto error_regulator;
	}
	else {
		err = regulator_set_voltage(cd->sensor_vdd, 1800000, 1800000);
		if(err < 0){
			pr_err("%s: vdd: set_voltage Error. err = %d\n", __func__, err);
			prob_err_state = CY_REG_VOL;
			goto error_regulator;
		}
		err = regulator_set_optimum_mode(cd->sensor_vdd, 15000);
		if(err < 0){
			prob_err_state = CY_REG_OPT;
			pr_err("%s: vdd: set_optimum Error. err = %d\n", __func__, err);
			goto error_regulator;
		} 
		pr_info("%s: vdd: set_optimum = 0x%02d.\n", __func__, err);
		err = regulator_enable(cd->sensor_vdd);
		if(err){
			prob_err_state = CY_REG_EN;
			pr_err("%s: vdd: regulator_enable fail. err = %d\n", __func__, err);
			goto error_regulator;
		}
	}
	usleep_range(1000,1000);
...
int cyttsp5_shutdown(struct cyttsp5_core_data *cd)
{
	...
	regulator_disable(cd->sensor_vdd);
