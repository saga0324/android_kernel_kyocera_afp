...
static int cyttsp5_xy_worker(struct cyttsp5_btn_data *bd)
{
	...
	if(cd->ts_event_control){
		pr_notice("%s btn is skip\n", __func__);
		if(cd->touch_cnt)
			cyttsp5_btn_lift_all(bd);
		return 0;
	}

	timeout = jiffies + msecs_to_jiffies(STATUS_CHECK_TIMEOUT);
	while(!time_after(jiffies, timeout)){
		isLargeArea = IS_LARGE_AREA(md->si->xy_mode[5]);
		if(isLargeArea){
			pr_info("%s: Large touch event cancel\n", __func__);
			return 0;
		}
		usleep_range(1,1);
	}
