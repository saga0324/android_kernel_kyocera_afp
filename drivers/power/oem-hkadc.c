...
static int get_prop_xo_therm(struct oem_hkac_chip *chip)
{
	int rc = 0;
	struct qpnp_vadc_result results;
	...
	if (rc) {
		HKADC_ERR("Unable to read xo temperature rc=%d\n", rc);
		return DEFAULT_TEMP;
	}
	HKADC_DEBUG("xo therm %d, %lld\n", results.adc_code, results.physical);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_XO_THERM:
		val->intval = get_prop_xo_therm(chip);
		break;
...
static int oem_hkadc_probe(struct spmi_device *spmi)
{
	...
	pr_info("probe success:camera_therm = %d, pa_therm = %d, xo_therm = %d, usb_therm = %d\n",
					get_prop_camera_therm(chip), get_prop_pa_therm(chip),
					get_prop_xo_therm(chip), get_prop_usb_therm(chip));
	return 0;
