...
#endif
	bool			oem_radio_running;
	int				oem_radio_limit_ma;
};
...
#ifdef CONFIG_OEM_BATT_CARE_MODE
static void oem_set_batt_care_mode(struct smb135x_chg *chip);
#endif
...
static int oem_aicl_cancellation(struct smb135x_chg *chip);
/*================================================================ */
						...
						enum path_type path)
{
	...
	if (chip->oem_radio_running) {
		current_ma = min(current_ma, chip->oem_radio_limit_ma);
	}
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CARE_STATUS:
		...
		if (chip->oem_batt_care_status) {
			rc = smb135x_path_suspend(chip, DC, BATT_CARE_MODE, false);
			if (rc < 0) {
				pr_err("Couldn't set dc suspend rc %d\n", rc);
			}
			rc = smb135x_path_suspend(chip, USB, BATT_CARE_MODE, false);
			if (rc < 0) {
...
static int smb_parse_dt(struct smb135x_chg *chip)
{
	...
	if(oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_01, 6)) {
		rc = of_property_read_u32(node, "oem,factory-vfloat-mv",
							&chip->vfloat_mv);
	}
	if (rc < 0)
	...
	rc = of_property_read_u32(node, "oem,radio-limit-ma",
						&chip->oem_radio_limit_ma);
	if (rc < 0) {
		pr_err("radio-limit-ma property missing.\n");
	}
...
static void oem_set_batt_care_mode(struct smb135x_chg *chip)
{
	...
	rc = smb135x_path_suspend(chip, DC, BATT_CARE_MODE, true);
	if (rc < 0) {
		pr_err("Couldn't set dc suspend rc %d\n", rc);
	}
	rc = smb135x_path_suspend(chip, USB, BATT_CARE_MODE, true);
	if (rc < 0) {
		pr_err("Couldn't set usb suspend rc %d\n", rc);
	}
...
static int oem_aicl_cancellation(struct smb135x_chg *chip)
{
	...
	if (chip->oem_radio_running) {
		...
		rc = smb135x_masked_write(the_chip, CFG_D_REG,
				USBIN_AICL_EN_BIT, 0);
	} else {
...
void oem_current_limit_enable(bool enable)
{
	...
	the_chip->oem_radio_running = enable;
	rc = oem_aicl_cancellation(the_chip);
