...
#define CHG_VSYS_MASK			SMB135X_MASK(5, 4)
#define CHG_VSYS_3450MV_VAL			0x10
...
struct smb135x_chg {
	...
	bool				oem_limit_enable;
	int					oem_limit_current;
	int					oem_inhibit_delta_mv;
	int					oem_cradle_status_flag;
	bool				oem_audio_suspend_flag;
	bool				oem_step_control_flag;
	struct delayed_work	oem_step_control_work;
	unsigned int		oem_step_levels;
	unsigned int		oem_step_lvl_sel;
	unsigned int		*oem_cradle_step_ma;
	int					oem_vfloat_mv_now;
	bool				oem_rechg_check_flag;
	bool				oem_during_rechg_flag;
	struct delayed_work	oem_rechg_work;
...
static int oem_smb135x_chg_set_appropriate_iterm(struct smb135x_chg *chip);
...
static void oem_aicl_cancellation(struct smb135x_chg *chip, u32 reason, bool enable);

static int oem_chg_is_off_charge(void);
static void oem_cradle_step_current_control(struct smb135x_chg *chip);
...
#define OEM_STEP_CONTROL_WORK_MS	5000
static void oem_step_control_check_work(struct work_struct *work);

#define OEM_RECHG_CHECK_WORK_MS		60000
static void oem_rechg_check_work(struct work_struct *work);

static int oem_rechg_set(struct smb135x_chg *chip);

enum oem_cradle_chg_index {
	NO_FAKE = 0,
	FAKE_FULL_CRADLE,
};
/*================================================================ */
...
static int smb135x_get_prop_batt_status(struct smb135x_chg *chip)
{
	...
	if (chip->oem_cradle_status_flag == FAKE_FULL_CRADLE)
		return POWER_SUPPLY_STATUS_FULL;
						...
						int reason, bool suspend)
{
	...
	if (!(*path_suspended) && suspended) {
		...
		if (path == DC) {
			...
			pr_info("path suspend clear full_suppress\n");
		}
...
#ifdef CONFIG_OEM_BMS
	if (path == USB && suspend && reason != AUDIO) {
		oem_bms_set_full_suppress(false);
		pr_info("path suspend clear full_suppress\n");
	}
#endif
						...
						enum path_type path)
{
	...
	if (chip->oem_limit_enable && prop.intval == POWER_SUPPLY_TYPE_USB_CRADLE) {
		therm_ma = min(therm_ma, chip->oem_limit_current);
	}

	if (chip->oem_step_control_flag) {
		therm_ma = min(therm_ma, (int)chip->oem_cradle_step_ma[chip->oem_step_lvl_sel]);
	}
								...
								int lvl_sel)
{
	...
	if (chip->therm_lvl_sel == (chip->thermal_levels - 1)) {
		...
		pr_info("set to CHG_EN_ACTIVE_LOW_BIT:low\n");
		rc = smb135x_masked_write(chip, CFG_14_REG,
				CHG_EN_ACTIVE_LOW_BIT, 0);
		if (rc < 0) {
			pr_err("Couldn't set CHG_EN_ACTIVE_LOW_BIT rc=%d\n", rc);
		}

		pr_info("clear to fake_full flag\n");
		chip->oem_cradle_status_flag = NO_FAKE;
		chip->oem_during_rechg_flag = false;
...
static int smb135x_float_voltage_set(struct smb135x_chg *chip, int vfloat_mv)
{
	...
	chip->oem_vfloat_mv_now = vfloat_mv;
		...
		int resume_delta_mv)
{
	...
	if (!chip->inhibit_disabled) {
		resume_delta_mv = chip->oem_inhibit_delta_mv;
...
static void smb135x_external_power_changed(struct power_supply *psy)
{
	...
	chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_TYPE, &prop);
	if (prop.intval == POWER_SUPPLY_TYPE_USB_CRADLE) {
		if (chip->chg_done_batt_full) {
			pr_info("set to fake_full flag\n");
			chip->oem_cradle_status_flag = FAKE_FULL_CRADLE;
		}

		if (!oem_chg_is_off_charge() && !chip->oem_step_control_flag) {
			pr_info("step current control start\n");
			schedule_delayed_work(&chip->oem_step_control_work, 0);
		}
	}
	rc = oem_smb135x_chg_set_appropriate_iterm(chip);
...
static int chg_term_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	...
	if (!chip->iterm_disabled){
		...
		batt_temp = oem_smb135x_get_prop_batt_temp(chip);

		if (!!rt_stat && (batt_temp < chip->hot_bat_decidegc && batt_temp > chip->cold_bat_decidegc)) {
			oem_bms_set_full_suppress(true);

			pr_info("set to CHG_EN_ACTIVE_LOW_BIT:high\n");
			rc = smb135x_masked_write(chip, CFG_14_REG,
					CHG_EN_ACTIVE_LOW_BIT, CHG_EN_ACTIVE_LOW_BIT);
			if (rc < 0) {
				pr_err("Couldn't set CHG_EN_ACTIVE_LOW_BIT rc=%d\n", rc);
			}
...
static int taper_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	pr_debug("rt_stat = 0x%02x\n", rt_stat);
	if (chip->oem_step_control_flag) {
		oem_cradle_step_current_control(chip);
	}
	return 0;
}
...
static int handle_usb_removal(struct smb135x_chg *chip)
{
	...
	cancel_delayed_work_sync(&chip->oem_step_control_work);
	oem_cradle_step_current_control(chip);
	...
	chip->oem_rechg_check_flag = false;
	cancel_delayed_work_sync(&chip->oem_rechg_work);
...
static int handle_usb_insertion(struct smb135x_chg *chip)
{
	...
	if (!chip->oem_rechg_check_flag) {
		chip->oem_rechg_check_flag = true;
		schedule_delayed_work(&chip->oem_rechg_work, msecs_to_jiffies(OEM_RECHG_CHECK_WORK_MS));
	}
...
static int usbin_uv_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	...
	if (chip->usb_psy && !chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_TYPE, &prop)) {
		if ((prop.intval == POWER_SUPPLY_TYPE_USB_DCP) || (prop.intval == POWER_SUPPLY_TYPE_USB_CRADLE)) {
...
static int chg_inhibit_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	...
	if (!chip->inhibit_disabled
			&& !chip->usb_uv
			&& (batt_temp < chip->hot_bat_decidegc && batt_temp > chip->cold_bat_decidegc)
...
static int smb135x_hw_init(struct smb135x_chg *chip)
{
	...
	if (chip->inhibit_disabled)
		...
		reg = EN_CHG_INHIBIT_BIT | DISABLE_AUTO_RECHARGE_BIT;
	...
	/* set minimum VSYS voltage */
	rc = smb135x_masked_write(chip, CFG_4_REG,
				CHG_VSYS_MASK, CHG_VSYS_3450MV_VAL);
	if (rc < 0) {
		dev_err(chip->dev, "cannot write to config 4 rc = %d\n", rc);
	}
...
static int smb_parse_dt(struct smb135x_chg *chip)
{
	...
	rc = of_property_read_u32(node, "oem,inhibit-thresh-mv", &chip->oem_inhibit_delta_mv);
	if (rc < 0)
		chip->oem_inhibit_delta_mv = chip->resume_delta_mv;
	...
	rc = of_property_read_u32(node, "oem,cradle-limit-ma",
							&chip->oem_limit_current);
	if (rc < 0) {
		pr_err("cradle-limit-ma property missing.\n");
	}

	if (of_find_property(node, "oem,cradle-step-ma",
					&chip->oem_step_levels)) {
		chip->oem_cradle_step_ma = devm_kzalloc(chip->dev,
			chip->oem_step_levels,
			GFP_KERNEL);

		if (chip->oem_cradle_step_ma == NULL) {
			pr_err("cradle step ma kzalloc() failed.\n");
			return -ENOMEM;
		}

		chip->oem_step_levels /= sizeof(int);
		rc = of_property_read_u32_array(node,
				"oem,cradle-step-ma",
				chip->oem_cradle_step_ma, chip->oem_step_levels);
		if (rc) {
			pr_err("Couldn't read cradle step ma rc = %d\n", rc);
			return rc;
		}
	}
				...
				const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->oem_step_control_work, oem_step_control_check_work);
	INIT_DELAYED_WORK(&chip->oem_rechg_work, oem_rechg_check_work);
	...
	if (!oem_chg_is_off_charge() && !oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_03, 2)) {
		rc = oem_rechg_set(chip);
		if (rc < 0) {
			pr_err("Unable to oem_rechg_set\n");
		}
	}
...
static int oem_smb135x_chg_set_appropriate_iterm(
				struct smb135x_chg *chip)
{
	int rc;
	unsigned int iterm = CHG_ITERM_100MA;
	union power_supply_propval prop = {0, };

	chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_TYPE, &prop);
	if (prop.intval == POWER_SUPPLY_TYPE_USB_CRADLE && !oem_chg_is_off_charge())
		iterm = CHG_ITERM_250MA;
	else if (chip->batt_cool)
		iterm = CHG_ITERM_150MA;

	pr_debug("setting 0x%X\n", iterm);
	rc = smb135x_masked_write(chip, CFG_3_REG,
							CHG_ITERM_MASK, iterm);
	if (rc) {
		dev_err(chip->dev,
			"Couldn't set iterm rc = %d\n", rc);
	}
...
static void oem_aicl_cancellation(struct smb135x_chg *chip, u32 reason, bool enable)
{
	static bool step_control_status = 0;
	static bool audio_status = 0;
	int rc = 0;

	if (reason == STEP_CONTROL) {
		step_control_status = enable;
	}
	if (reason == AUDIO) {
		audio_status = enable;
	}

	if (!step_control_status && !audio_status) {
		pr_debug("AICL enable\n");
		rc = smb135x_masked_write(chip, CFG_D_REG,
				USBIN_AICL_EN_BIT, USBIN_AICL_EN_BIT);
	} else {
		pr_debug("AICL disnable\n");
		rc = smb135x_masked_write(chip, CFG_D_REG,
				USBIN_AICL_EN_BIT, 0);
	}
}

void oem_current_limit_enable(bool enable)
{
	int rc = 0;
	int status = smb135x_get_prop_batt_status(the_chip);

	mutex_lock(&the_chip->current_change_lock);

	oem_aicl_cancellation(the_chip, AUDIO, enable);

	if (enable) {
		if (the_chip->therm_lvl_sel == (the_chip->thermal_levels - 1)
				|| status == POWER_SUPPLY_STATUS_FULL
				|| oem_bms_get_full_suppress()) {
			if (!the_chip->oem_during_rechg_flag) {
				the_chip->oem_audio_suspend_flag = true;
				pr_debug("AUDIO suspend enable\n");
				rc = smb135x_path_suspend(the_chip, USB, AUDIO, true);
			}
		}
	} else {
		pr_debug("AUDIO suspend disnable\n");
		rc = smb135x_path_suspend(the_chip, USB, AUDIO, false);
		the_chip->oem_audio_suspend_flag = false;
	}

	the_chip->oem_limit_enable = enable;
	rc = smb135x_set_appropriate_current(the_chip, USB);

	mutex_unlock(&the_chip->current_change_lock);
}
EXPORT_SYMBOL(oem_current_limit_enable);
...
static void
oem_step_control_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb135x_chg *chip = container_of(dwork, struct smb135x_chg, oem_step_control_work);

	if (chip->usb_present) {
		oem_cradle_step_current_control(chip);
		schedule_delayed_work(&chip->oem_step_control_work, msecs_to_jiffies(OEM_STEP_CONTROL_WORK_MS));
	}
}

#define STEP_UP_THRES_UV	300000
#define STEP_DOWN_THRES_UV	20000
static void oem_cradle_step_current_control(struct smb135x_chg *chip)
{
	int rc = 0;
	int batt_volt;
	...
	if (!chip->usb_present) {
		chip->oem_step_control_flag = false;
		pr_info("charger is not present\n");
		chip->oem_step_lvl_sel = (chip->oem_step_levels - chip->oem_step_levels);
		goto set_current;
	}

	chip->oem_step_control_flag = true;

	batt_volt = oem_smb135x_get_prop_battery_voltage_now(chip);
	if (batt_volt <= (chip->oem_vfloat_mv_now * 1000 - STEP_UP_THRES_UV)) {
		if (chip->oem_step_lvl_sel < (chip->oem_step_levels - 1)) {
			chip->oem_step_lvl_sel++;
		}
	} else if (batt_volt >= (chip->oem_vfloat_mv_now * 1000 - STEP_DOWN_THRES_UV)) {
		if (chip->oem_step_lvl_sel > (chip->oem_step_levels - chip->oem_step_levels)) {
			chip->oem_step_lvl_sel--;
		} else if (chip->oem_step_lvl_sel == (chip->oem_step_levels - chip->oem_step_levels)
					&& chip->oem_limit_enable
					&& !(chip->batt_hot || chip->batt_cold)) {
			oem_bms_set_full_suppress(true);
			...
			chip->oem_audio_suspend_flag = true;
			pr_info("AUDIO suspend enable\n");
			rc = smb135x_path_suspend(chip, USB, AUDIO, true);
		}
	} else {
		goto unlock;
	}
...
set_current:
	oem_aicl_cancellation(chip, STEP_CONTROL, chip->oem_step_control_flag);
	pr_info("set to currert:%d\n", chip->oem_cradle_step_ma[chip->oem_step_lvl_sel]);
	rc = smb135x_set_appropriate_current(chip, USB);
...
#define OEM_RECHG_VOLT_THRESH_CRADLE_NORMAL		4000000
#define OEM_RECHG_VOLT_THRESH_CRADLE			3800000
#define OEM_RECHG_VOLT_THRESH_NORMAL			4100000
#define OEM_RECHG_VOLT_THRESH					4000000
static void
oem_rechg_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb135x_chg *chip = container_of(dwork, struct smb135x_chg, oem_rechg_work);
	union power_supply_propval prop = {0,};
	int rc;
	int voltage = oem_smb135x_get_prop_battery_voltage_now(chip);
	int health = smb135x_get_prop_batt_health(chip);
	bool rechg_start_flag = false;
	...
	chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_TYPE, &prop);
	if (prop.intval == POWER_SUPPLY_TYPE_USB_CRADLE && chip->oem_audio_suspend_flag) {
		if (voltage <= OEM_RECHG_VOLT_THRESH_CRADLE_NORMAL && health == POWER_SUPPLY_HEALTH_GOOD) {
			rechg_start_flag = true;
		}

		if (voltage <= OEM_RECHG_VOLT_THRESH_CRADLE) {
			rechg_start_flag = true;
		}
	} else {
		if (voltage <= OEM_RECHG_VOLT_THRESH_NORMAL && health == POWER_SUPPLY_HEALTH_GOOD) {
			rechg_start_flag = true;
		}

		if (voltage <= OEM_RECHG_VOLT_THRESH) {
			rechg_start_flag = true;
		}
	}

	if (rechg_start_flag) {
		pr_info("recharge start\n");
		chip->oem_during_rechg_flag = true;

		pr_info("AUDIO suspend disable to recharge\n");
		rc = smb135x_path_suspend(chip, USB, AUDIO, false);
		chip->oem_audio_suspend_flag = false;
		...
		if (oem_bms_get_full_suppress()) {
			pr_info("Since status is Charging, clear flag\n");
			chip->oem_cradle_status_flag = NO_FAKE;
			oem_bms_set_full_suppress(false);
		}
	}
...
static int oem_rechg_set(struct smb135x_chg *chip)
{
	...
	if (!chip->inhibit_disabled) {
		rc = smb135x_masked_write(chip, CFG_14_REG, EN_CHG_INHIBIT_BIT,
					0);
		if (rc) {
			pr_err("Couldn't disable charge-inhibit rc=%d\n", rc);
		}
		...
		rc |= smb135x_masked_write(chip, CFG_14_REG,
				EN_CHG_INHIBIT_BIT, EN_CHG_INHIBIT_BIT);
		if (rc) {
			pr_err("Couldn't enable charge-inhibit rc=%d\n", rc);
		}
	}
