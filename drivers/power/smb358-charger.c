...
#define CHG_CTRL_PRE_CHG_TIMEOUT_BIT	BIT(1)
...
enum {
	...
	CURRENT = BIT(2),
	SOC	= BIT(3),
#ifdef CONFIG_OEM_BATT_CARE_MODE
	BATT_CARE_MODE = BIT(29),
#endif
#ifdef CONFIG_OEM_HKADC_USB_TM
	USB_THERMAL = BIT(30),
#endif
#ifdef CONFIG_OEM_SYSTEM_TEMP_LEVEL
	OEM_SYSTEM_TEMP_LEVEL	= BIT(31),
#endif
};
...
#ifndef CONFIG_OEM_SYSTEM_TEMP_LEVEL
					...
					const union power_supply_propval *val)
{
...
#ifdef CONFIG_OEM_HKADC_USB_TM
	case POWER_SUPPLY_PROP_USB_TEMP_LEVEL:
		smb358_path_suspend(chip, USB_THERMAL, val->intval);
		break;
#endif
#ifdef CONFIG_OEM_BATT_CARE_MODE
	case POWER_SUPPLY_PROP_BATTERY_CARE_MODE:
	...
	case POWER_SUPPLY_PROP_BATTERY_CARE_STATUS:
		...
		if (chip->oem_batt_care_status) {
			rc = smb358_path_suspend(chip, BATT_CARE_MODE, false);
			if (rc < 0) {
...
static void oem_set_chg_wakelock(int present)
{
	if(present) {
...
static int apsd_complete(struct smb358_charger *chip, u8 status)
{
	...
	oem_set_chg_wakelock(chip->chg_present);
				...
				struct smb358_charger *chip)
{
	...
	if (chip->therm_lvl_sel > 0
		 ...
		 * the highest level
		 */
		current_max = min(current_max, chip->thermal_mitigation[chip->therm_lvl_sel]);
#endif
...
static int smb_parse_dt(struct smb358_charger *chip)
{
	...
	if (oem_bms_get_deterioration_status() >= DETERIORATION_STATUS_GOOD
		...
		rc = of_property_read_u32(node, "oem,deterioration-vfloat-mv",
							&chip->vfloat_mv);
		if (rc < 0) {
			chip->vfloat_mv = chip->warm_bat_mv;
...
static void oem_set_batt_care_mode(struct smb358_charger *chip)
{
	...
	rc = smb358_path_suspend(chip, BATT_CARE_MODE, true);
	if (rc < 0) {
		pr_err("Couldn't set suspend rc %d\n", rc);
	}
