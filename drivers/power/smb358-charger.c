...
#define LIMIT_CELL_TEMP_REG		0xB
...
#define CHG_CTRL_COMPLETE_CHG_TIMEOUT_BIT	BIT(3)
#define CHG_CTRL_COMPLETE_CHG_TIMEOUT_MASK	(BIT(3) | BIT(2))
#define CHG_CTRL_PRE_CHG_TIMEOUT_BIT	BIT(0)
#define CHG_CTRL_PRE_CHG_TIMEOUT_MASK	(BIT(1) | BIT(0))
#define LIMIT_CELL_HW_COLD_BIT	BIT(6)
#define LIMIT_CELL_HW_COLD_MASK	(BIT(7) | BIT(6))
#define LIMIT_CELL_HW_HOT_BIT	BIT(5)
#define LIMIT_CELL_HW_HOT_MASK	(BIT(5) | BIT(4))
#define LIMIT_CELL_HW_HOT_BIT_OEM	BIT(4)
...
enum {
	...
	FAKE_BATTERY	= BIT(4),
	OEM_COMMAND	= BIT(5),
	OEM_SYSTEM_TEMP_LEVEL	= BIT(6),
	OEM_BATT_CARE_MODE = BIT(7),
#ifdef CONFIG_OEM_HKADC_USB_TM
	OEM_USB_THERMAL = BIT(8),
#endif
};
...
struct smb358_charger {
	...
	int						oem_batt_care_status;
	unsigned int			oem_thermal_levels;
	unsigned int			oem_therm_lvl_sel;
	unsigned int			*oem_thermal_mitigation;
	unsigned int			oem_batt_care_mv;
	bool					oem_battery_ov;
	struct mutex			oem_current_change_lock;
	struct wake_lock		oem_charge_wake_lock;
	struct delayed_work		oem_batt_care_check_work;
#ifdef CONFIG_OEM_BMS
...
/*-------*/
#define OEM_FACTORY_USB_ICL            1000
#define DNAND_OFFSET_BATT_CARE_MODE		14
#define DNAND_OFFSET_BATT_CARE_STATUS	15
...
static int oem_smb358_system_temp_level_set(struct smb358_charger *chip, int lvl_sel);
static void oem_smb358_suspend_mode_is_disable(struct smb358_charger *chip);
static void oem_set_chg_wakelock(struct smb358_charger *chip);
static void oem_set_batt_care_mode(struct smb358_charger *chip);
static void oem_batt_care_check_work(struct work_struct *work);
static void oem_batt_care_dnand_backup(struct smb358_charger *chip);
static int oem_get_prop_current_now(struct smb358_charger *chip);
static int oem_get_prop_charge_counter(struct smb358_charger *chip);
#ifdef CONFIG_OEM_BMS
static void oem_recharge_flag_init(struct smb358_charger *chip);
...
static int smb358_hw_init(struct smb358_charger *chip)
{
	...
	/* set CHG TIMEOUT */
	reg = CHG_CTRL_COMPLETE_CHG_TIMEOUT_BIT | CHG_CTRL_PRE_CHG_TIMEOUT_BIT;
	mask =  CHG_CTRL_COMPLETE_CHG_TIMEOUT_MASK | CHG_CTRL_PRE_CHG_TIMEOUT_MASK;
	rc = smb358_masked_write(chip, STAT_AND_TIMER_CTRL_REG, mask, reg);
	if (rc) {
		dev_err(chip->dev, "Couldn't set STAT_AND_TIMER_CTRL_REG rc=%d\n", rc);
		return rc;
	}

	/* set H/W S/W Limit Cell Temperature Monitor */
	reg = LIMIT_CELL_HW_COLD_BIT | LIMIT_CELL_HW_HOT_BIT_OEM;
	mask =  LIMIT_CELL_HW_COLD_MASK | LIMIT_CELL_HW_HOT_MASK;
	rc = smb358_masked_write(chip, LIMIT_CELL_TEMP_REG, mask, reg);
	if (rc) {
		dev_err(chip->dev, "Couldn't set LIMIT_CELL_TEMP_REG rc=%d\n", rc);
		return rc;
	}
...
static int smb358_get_prop_batt_status(struct smb358_charger *chip)
{
	...
	if ((chip->batt_full || chip->oem_fake_batt_full) && !oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_01, 6))
					...
					enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CARE_STATUS:
#ifdef CONFIG_OEM_HKADC_USB_TM
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
		...
		oem_smb358_system_temp_level_set(chip, val->intval);
		break;
	case POWER_SUPPLY_PROP_OEM_INPUT_SUSPEND:
		rc = smb358_path_suspend(chip, OEM_COMMAND, !!val->intval);
		break;
	case POWER_SUPPLY_PROP_OEM_BATTFET_SUSPEND:
		rc = smb358_charging_disable(chip, OEM_COMMAND, !!val->intval);
		break;
	case POWER_SUPPLY_PROP_BATTERY_CARE_MODE:
		if (chip->oem_batt_care_mode != val->intval) {
			chip->oem_batt_care_mode = val->intval;
			if (!chip->oem_batt_care_mode) {
				chip->oem_batt_care_status = chip->oem_batt_care_mode;
			}
			oem_set_batt_care_mode(chip);
			rc = smb358_get_prop_batt_capacity(chip);
			power_supply_changed(&chip->batt_psy);

			oem_batt_care_dnand_backup(chip);
		}
		break;
	case POWER_SUPPLY_PROP_BATTERY_CARE_STATUS:
		if (chip->oem_batt_care_status != val->intval) {
			chip->oem_batt_care_status = val->intval;
			if (chip->oem_batt_care_status) {
				rc = smb358_path_suspend(chip, OEM_BATT_CARE_MODE, false);
				if (rc < 0) {
					pr_err("Couldn't set suspend rc %d\n", rc);
				}
			}
...
#ifdef CONFIG_OEM_HKADC_USB_TM
	case POWER_SUPPLY_PROP_USB_TEMP_LEVEL:
		smb358_path_suspend(chip, OEM_USB_THERMAL, val->intval);
		break;
#endif
	default:
		return -EINVAL;
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		break;
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
		val->intval = chip->oem_therm_lvl_sel;
		break;
	case POWER_SUPPLY_PROP_OEM_THERMAL_LEVELS:
		val->intval = chip->oem_thermal_levels;
		break;
	case POWER_SUPPLY_PROP_OEM_INPUT_SUSPEND:
		val->intval = !!(chip->usb_suspended & OEM_COMMAND);
		break;
	case POWER_SUPPLY_PROP_OEM_BATTFET_SUSPEND:
		val->intval = !!(chip->charging_disabled_status & OEM_COMMAND);
		break;
	case POWER_SUPPLY_PROP_BATTERY_CARE_MODE:
		val->intval = chip->oem_batt_care_mode;
		break;
	case POWER_SUPPLY_PROP_BATTERY_CARE_STATUS:
		val->intval = chip->oem_batt_care_status;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = oem_get_prop_current_now(chip);
		break;
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval = oem_get_prop_charge_counter(chip);
		break;
#ifdef CONFIG_OEM_HKADC_USB_TM
	case POWER_SUPPLY_PROP_USB_TEMP_LEVEL:
				...
				struct smb358_charger *chip)
{
	...
	if (chip->oem_therm_lvl_sel > 0
			&& chip->oem_therm_lvl_sel < (chip->oem_thermal_levels - 1))
		/*
		 * consider thermal limit only when it is active and not at
		 * the highest level
		 */
		current_max = min(current_max, chip->oem_thermal_mitigation[chip->oem_therm_lvl_sel]);
				...
				struct smb358_charger *chip)
{
	...
	if (chip->oem_batt_care_mode)
		vddmax = min(vddmax, chip->oem_batt_care_mv);
...
static int battery_ov_handler(struct smb358_charger *chip, u8 status)
{
	pr_debug("battery ov rt_stat = 0x%02x\n", status);
	chip->oem_battery_ov = !!status;
	return 0;
}
...
static int smb_parse_dt(struct smb358_charger *chip)
{
	...
	if (of_find_property(node, "qcom,thermal-mitigation",
					&chip->oem_thermal_levels)) {
		chip->oem_thermal_mitigation = devm_kzalloc(chip->dev,
			chip->oem_thermal_levels,
			GFP_KERNEL);

		if (chip->oem_thermal_mitigation == NULL) {
			pr_err("thermal mitigation kzalloc() failed.\n");
			return -ENOMEM;
		}

		chip->oem_thermal_levels /= sizeof(int);
		rc = of_property_read_u32_array(node,
				"qcom,thermal-mitigation",
				chip->oem_thermal_mitigation, chip->oem_thermal_levels);
		if (rc) {
	...
	rc = of_property_read_u32(node, "oem,batt-care-mv",
						&chip->oem_batt_care_mv);
	if (rc < 0) {
		chip->oem_batt_care_mv = chip->vfloat_mv;
		pr_err("batt-care-mv property missing.\n");
	}
				...
				const struct i2c_device_id *id)
{
	...
	mutex_init(&chip->oem_current_change_lock);

	oem_smb358_suspend_mode_is_disable(chip);
	...
	if(oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_03, 2) && chip->chg_present){
		smb358_set_usb_chg_current(chip, OEM_FACTORY_USB_ICL);
	}
	...
	wake_lock_init(&chip->oem_charge_wake_lock, WAKE_LOCK_SUSPEND, "oem_charge");
	...
	INIT_DELAYED_WORK(&chip->oem_batt_care_check_work, oem_batt_care_check_work);
	schedule_delayed_work(&chip->oem_batt_care_check_work,
			msecs_to_jiffies(DNAND_CHECK_CYCLE));
...
/*-------*/
static int oem_smb358_system_temp_level_set(struct smb358_charger *chip,
								int lvl_sel)
{
	int rc = 0;
	int prev_therm_lvl;

	if (!chip->oem_thermal_mitigation) {
	...
	if (lvl_sel >= chip->oem_thermal_levels) {
		pr_err("Unsupported level selected %d forcing %d\n", lvl_sel,
				chip->oem_thermal_levels - 1);
		lvl_sel = chip->oem_thermal_levels - 1;
	}

	if (lvl_sel == chip->oem_therm_lvl_sel)
		return 0;

	mutex_lock(&chip->oem_current_change_lock);
	prev_therm_lvl = chip->oem_therm_lvl_sel;
	chip->oem_therm_lvl_sel = lvl_sel;

	if (chip->oem_therm_lvl_sel == (chip->oem_thermal_levels - 1)) {
		/*
		 * Disable charging if highest value selected by
		 * setting path in suspend
		 */
		rc = smb358_path_suspend(chip, OEM_SYSTEM_TEMP_LEVEL, true);
		if (rc < 0) {
			dev_err(chip->dev,
				"Couldn't set suspend rc %d\n", rc);
		}
		goto out;
	}
	...
	if (prev_therm_lvl == chip->oem_thermal_levels - 1) {
		/*
		 * If previously highest value was selected charging must have
		 * been disabed. Enable charging by taking path
		 * out of suspend.
		 */
		rc = smb358_path_suspend(chip, OEM_SYSTEM_TEMP_LEVEL, false);
		if (rc < 0) {
			dev_err(chip->dev,
...
out:
	mutex_unlock(&chip->oem_current_change_lock);
	return rc;
}

static int oem_smb358_suspend_mode_check(struct smb358_charger *chip)
{
	int rc = 0;
	u8 reg = 0;
	...
	if (rc) {
		dev_err(chip->dev,
			"Couldn't read SUSP enable bit rc=%d, reg=%2x\n",
							rc, CMD_A_REG);
		return rc;
	}

	return  (reg & CMD_A_CHG_SUSP_EN_BIT) ? 1 : 0;
}

static void oem_smb358_suspend_mode_is_disable(struct smb358_charger *chip)
{
	int rc = 0;
	int oem_suspend_mode;

	mutex_lock(&chip->path_suspend_lock);

	oem_suspend_mode = oem_smb358_suspend_mode_check(chip);

	if (oem_suspend_mode && !chip->usb_suspended){
		rc = __smb358_path_suspend(chip, false);
		if (rc < 0) {
			dev_err(chip->dev,
				"Couldn't unset suspend rc %d\n", rc);
		}
	}
...
{
	if(chip->chg_present) {
		wake_lock(&chip->oem_charge_wake_lock);
		pr_debug("SET wakelock %d\n", chip->chg_present);
	}else {
		wake_unlock(&chip->oem_charge_wake_lock);
		pr_debug("CLEAR wakelock %d\n", chip->chg_present);
	}
}
...
static void oem_set_batt_care_mode(struct smb358_charger *chip)
{
	...
	rc = smb358_path_suspend(chip, OEM_BATT_CARE_MODE, true);
	if (rc < 0) {
		pr_err("Couldn't set suspend rc %d\n", rc);
	}
	...
	if (chip->oem_batt_care_mode && !chip->oem_batt_care_status) {
		return;
	}
...
static void oem_batt_care_check_work(struct work_struct *work)
{
	struct smb358_charger *chip = container_of(work,
				struct smb358_charger, oem_batt_care_check_work.work);
	u8 mode = BATT_CARE_OFF;
	u8 status = BATT_CARE_OFF;
	int ret = 0, ret_2 = 0;
	int rc = 0;
	union power_supply_propval prop = {0,};
	static int read_err_count = 0;

	ret = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_MODE,
		(uint8_t*)&mode, sizeof(mode));
	ret_2 = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_STATUS,
		(uint8_t*)&status, sizeof(status));
	if (ret == DNAND_NO_ERROR && ret_2 == DNAND_NO_ERROR) {
		pr_debug("kdnand_id_read succeed\n");

		if (mode == BATT_CARE_ON) {
			prop.intval = mode;
			rc = smb358_battery_set_property(&chip->batt_psy,
					POWER_SUPPLY_PROP_BATTERY_CARE_MODE, &prop);
		}

		if (status == BATT_CARE_ON) {
			prop.intval = status;
			rc = smb358_battery_set_property(&chip->batt_psy,
					POWER_SUPPLY_PROP_BATTERY_CARE_STATUS, &prop);
		}
	} else {
		pr_debug("kdnand_id_read error:%d, %d\n", ret, ret_2);
		read_err_count++;

		if (read_err_count > DNAND_READ_ERR_COUNT) {
			pr_info("DNAND clear, read_err_count:%d\n", read_err_count);
			mode = BATT_CARE_OFF;
			status = BATT_CARE_OFF;
			ret = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_MODE,
				(uint8_t*)&mode, sizeof(mode));
			ret_2 = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_STATUS,
				(uint8_t*)&status, sizeof(status));
		} else {
			schedule_delayed_work(&chip->oem_batt_care_check_work,
					msecs_to_jiffies(DNAND_CHECK_CYCLE));
...
{
	int ret = 0;
	u8 mode = chip->oem_batt_care_mode;
	u8 status = chip->oem_batt_care_status;

	ret = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_MODE,
		(uint8_t*)&mode, sizeof(mode));
	ret = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_STATUS,
		(uint8_t*)&status, sizeof(status));
}
...
static void oem_recharge_enable(struct smb358_charger *chip)
{
	int rc = 0;
...
static void oem_recharge_check(struct work_struct *work)
{
	int temp = 0, status = 0;
	struct smb358_charger *chip = container_of(work,
				struct smb358_charger, oem_recharge_work.work);
