...
#define POWER_OFF_THRESHOLD_UV_LOW_TEMP		(3100 * 1000)
...
#define BATT_THERM_SAMPLES				5
#define BATT_INITIAL_TEMP				-110
#define LOW_TEMP_THRESHOLD				-110
#define LOW_TEMP_THRESHOLD_HYSTERESIS	20
...
struct oem_therm_average {
	int index;
	int buf[BATT_THERM_SAMPLES];
};
static struct oem_therm_average batt_therm = {
	.index = 0,
	.buf = { BATT_INITIAL_TEMP, BATT_INITIAL_TEMP, BATT_INITIAL_TEMP, BATT_INITIAL_TEMP, BATT_INITIAL_TEMP }
};
...
static int shutdown_uv = POWER_OFF_THRESHOLD_UV_LOW_TEMP;
...
static int oem_bms_check_low_vbatt(int soc, int battery_status)
{
	...
	if(vbatt_current_uv <= shutdown_uv) {
		result_soc = POWER_OFF_SOC;
		pr_err("set auto power off soc:%d vbatt_uv:%d shutdown_uv:%d\n", soc, vbatt_current_uv, shutdown_uv);
...
static void oem_change_low_temp_shutdown_uv(bool low_temp_detection)
{
	if (low_temp_detection) {
		shutdown_uv = POWER_OFF_THRESHOLD_UV_LOW_TEMP;
	} else {
		shutdown_uv = POWER_OFF_THRESHOLD_UV;
	}
	pr_debug("Change of shutdown voltage:%d\n", shutdown_uv);
}

void oem_check_batt_therm(void)
{
	static bool low_temp_detection = true;
	int batt_temp = oem_bms_get_batt_prop(POWER_SUPPLY_PROP_TEMP);
	int batt_temp_average;

	batt_therm.buf[batt_therm.index] = batt_temp;
	batt_therm.index++;
	batt_therm.index %= BATT_THERM_SAMPLES;

	batt_temp_average = oem_bms_get_average(batt_therm.buf, BATT_THERM_SAMPLES);
	pr_debug("average:%d [%d][%d][%d][%d][%d]\n",
		batt_temp_average, batt_therm.buf[0], batt_therm.buf[1], batt_therm.buf[2], batt_therm.buf[3], batt_therm.buf[4]);

	if (low_temp_detection) {
		if (batt_temp_average >= (LOW_TEMP_THRESHOLD + LOW_TEMP_THRESHOLD_HYSTERESIS)) {
			pr_info("return from low temperature\n");
			low_temp_detection = false;
			oem_change_low_temp_shutdown_uv(low_temp_detection);
		}
	} else {
		if (batt_temp_average <= LOW_TEMP_THRESHOLD) {
			pr_info("Detection of low temperature\n");
			low_temp_detection = true;
			oem_change_low_temp_shutdown_uv(low_temp_detection);
		}
	}
}
