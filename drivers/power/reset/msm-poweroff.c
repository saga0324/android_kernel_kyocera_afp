...
static int oem_pw_sw_check_flg = 0;
module_param(oem_pw_sw_check_flg, int, 0644);
...
static void do_msm_poweroff(void)
{
    ...
    *reset_flag = 0;
	...
	if(oem_pw_sw_check_flg) {
		pw_sw = gpio_get_value(pw_sw_gpio);
		pr_err("pw_sw_gpio[%d] = %d\n", pw_sw_gpio, pw_sw);
		if(pw_sw == 0) {
			pr_notice("Going down for restart now by pw_sw\n");

			do_msm_restart(REBOOT_HARD, NULL);
	    }
	}

	pr_notice("Going down for poweroff now\n");
...
static int msm_restart_probe(struct platform_device *pdev)
{
	...
	pw_sw_gpio = of_get_named_gpio(pdev->dev.of_node, "pw_sw-gpio", 0);
