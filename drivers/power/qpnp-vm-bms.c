...
static int backup_ocv_soc(struct qpnp_bms_chip *chip, int ocv_uv, int soc)
{
	...
	if ( (rc == 0) && (batt_temp <= BATT_REMOVED_TEMP) ) {
		pr_err("NOT backup OCV at %d\n", batt_temp);
		return 0;
	}
...
static int backup_charge_cycle(struct qpnp_bms_chip *chip)
{
	...
	ret = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYCLE_COUNT,
		(uint8_t*)&chip->charge_cycles, sizeof(chip->charge_cycles));
	ret_2 = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYC_CTR_CHG_INC,
		(uint8_t*)&chip->charge_increase, sizeof(chip->charge_increase));
	if (ret == DNAND_NO_ERROR && ret_2 == DNAND_NO_ERROR) {
		pr_debug("kdnand_id_write succeed\n");
	} else {
		pr_debug("kdnand_id_write error:%d, %d\n", ret, ret_2);
		rc = EINVAL;
	}
#endif
...
static int read_chgcycle_data_from_backup(struct qpnp_bms_chip *chip)
{
	...
	ret = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYCLE_COUNT,
		(uint8_t*)&chip->charge_cycles, sizeof(chip->charge_cycles));
	rc = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYC_CTR_CHG_INC,
		(uint8_t*)&chip->charge_increase, sizeof(chip->charge_increase));
	if (ret == DNAND_NO_ERROR && ret_2 == DNAND_NO_ERROR) {
		pr_debug("kdnand_id_read succeed\n");
		chip->ome_dnand_init = true;
	} else {
		pr_debug("kdnand_id_read error:%d, %d\n", ret, ret_2);
		rc = EINVAL;
		chip->charge_cycles = DEFAULT_CYCLE_COUNT;
		chip->charge_increase = DEFAULT_CHG_INC;
	}
#endif
...
#ifndef QUALCOMM_ORIGINAL_FEATURE
	if (!chip->ome_dnand_init) {
		rc = read_chgcycle_data_from_backup(chip);
		if (rc) {
			pr_debug("charge_cycles read error:%d\n", rc);
			return;
		}
	}
#endif
...
#define SOC_CATCHUP_SEC_MAX		200
#define SOC_CATCHUP_SEC_PER_PERCENT	20
	...
	if (!is_battery_present(chip)) {
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			if (chip->calculated_soc != new_soc) {
				...
				new_report_soc = report_vm_bms_soc(chip);

				if (new_report_soc != last_report_soc) {
					pr_debug("update bms_psy\n");
					power_supply_changed(&chip->bms_psy);
				}
			}
			last_report_soc = report_vm_bms_soc(chip);
		}
...
static enum power_supply_property bms_power_props[] = {
	...
	POWER_SUPPLY_PROP_OEM_LEARNING_MAH,
#endif
};
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_BMS_BATT_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_OEM_LEARNING_MAH:
		val->intval = oem_bms_get_learning_mah();
		break;
#endif
	default:
		return -EINVAL;
