...
static ssize_t kgsl_pwrctrl_idle_timer_en_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	unsigned int val = 0;
	...
	device->pwrctrl.interval_timeout_en = val;
...
static ssize_t kgsl_pwrctrl_idle_timer_en_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct kgsl_device *device = kgsl_device_from_dev(dev);
	if (device == NULL)
		return 0;
	return snprintf(buf, PAGE_SIZE, "%d\n",
		device->pwrctrl.interval_timeout_en);
}
...
static DEVICE_ATTR(idle_timer_en, 0644, kgsl_pwrctrl_idle_timer_en_show,
	kgsl_pwrctrl_idle_timer_en_store);
...
void kgsl_idle_check(struct work_struct *work)
{
	...
	if (snd_soc_get_ic_recoder_active()) {
		if (device->pwrctrl.interval_timeout_en)
			device->pwrctrl.interval_timeout_en = 0;
	} else {
		if (!device->pwrctrl.interval_timeout_en) {
			device->pwrctrl.interval_timeout_en = 1;
		}
	}
