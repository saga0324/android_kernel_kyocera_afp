...
#define MSM_CAMERA_OTP_ADDR 0x27DF3000
#define MSM_CAMERA_OTP_BYTE 40
#define MSM_CAMERA_PDAF_ADDR 0x27DF34D4
#define MSM_CAMERA_PDAF_BYTE 180

#define CAMERA_DRIVER_NAME "/dev/video1"
#define CAMERA_SENSOR_NAME "/dev/v4l-subdev4"
...
enum msm_camera_stream_type_t {
	...
	MSM_CAMERA_STREAM_PREVIEW_SNAPSHOT,
...
struct sensor_fps_info_t {
    int32_t min_fps;
    int32_t max_fps;
    int32_t video_min_fps;
...
enum msm_sensor_led_ctrl {
	CAMERA_LED_INVALID = -1,
	CAMERA_LED_OFF = 0,
	CAMERA_LED_AUTO,
	CAMERA_LED_ON,
	CAMERA_LED_TORCH,
	CAMERA_LED_HAL_OFF,
	CAMERA_LED_TORCH_HIGH,
	CAMERA_LED_TORCH_MID,
	CAMERA_LED_TORCH_LOW,
};

enum msm_sensor_antibanding_mode {
    CAMERA_ANTIBANDING_MODE_AUTO,
    CAMERA_ANTIBANDING_MODE_50HZ,
    CAMERA_ANTIBANDING_MODE_60HZ,
    CAMERA_ANTIBANDING_MODE_OFF,
};

enum msm_sensor_iso_mode {
    ISO_MODE_AUTO,
    ISO_MODE_DEBLUR,
    ISO_MODE_100,
    ISO_MODE_200,
...
enum msm_sensor_auto_exposure_mode{
    AEC_MODE_FRAME_AVERAGE,
    AEC_MODE_CENTER_WEIGHTED,
    AEC_MODE_SPOT_METERING,
    AEC_MODE_SMART_METERING,
    AEC_MODE_USER_METERING,
    AEC_MODE_SPOT_METERING_ADV,
    AEC_MODE_CENTER_WEIGHTED_ADV,
    AEC_MODE_MAX
};

enum msm_sensor_stabilization_mode {
    CAMERA_STABILIZATION_MODE_OFF,
    CAMERA_STABILIZATION_MODE_ON,
};
...
struct sensor_exif_info_ex_t {
	uint32_t exposure_bias;
...
struct sensor_ispinfo_ex_t {
	int32_t size;
	uint8_t *data;
};

struct sensor_zoom_tbl_t {
	int32_t size;
	const uint32_t *data;
};

struct sensor_power_up_t {
	struct sensor_zoom_tbl_t zoom_table;
	int kyocera_camera_flg;
};

struct sensor_stream_type_info_t {
...
struct sensorb_cfg_data {
	...
	union {
		...
		struct sensor_fps_info_t      fps_info;
		uint32_t                      fd_mode;
	    int32_t                       led;
	    int32_t                       wb;
	    int32_t                       effect;
	    ...
	    int32_t                       contrast;
	    int32_t                       lock;
		struct sensor_ispinfo_ex_t    isp_info;
	    int32_t                       zoom;
		struct sensor_power_up_t      power_up;
...
enum msm_sensor_cfg_type_t {
	...
	CFG_SET_LED,
	CFG_SET_STREAM_TYPE,
	CFG_GET_EXIF_INFO_EX,
	CFG_GET_MAKERNOTE_ISPDATA_EX,
	CFG_SET_FW_UPDATE,
	CFG_SET_FW_ERASE,
	CFG_READ_OTP,
	CFG_READ_PDAF_OTP,
	CFG_WRITE_I2C_CATE_PARA,
	CFG_READ_I2C_CATE_PARA,
	CFG_SET_FACE_ROI,
	CFG_SET_FOCUS_MODE,
	CFG_SET_AF_ROI,
	CFG_SET_AEC_LOCK,
	...
	CFG_SET_WDR_EX,
	CFG_SET_FPS,
	CFG_SET_CONTINUOUS_SHOOT_EX,
	CFG_SET_ZOOM,
	CFG_SET_STABILIZATION_REDUCT,
	CFG_SET_BESTSHOT,
	CFG_GET_AUTO_SCENE,
	CFG_SET_UPDATE_SCENE,
	CFG_GET_LOWLIGHT_INTENSITY,
	CFG_SET_JPEG_QUALITY_EX,
	CFG_SET_FD,
	CFG_SET_AEC_ROI,
};
...
/* pdaf otp data is stored by big-endian */
typedef struct {
	uint8_t lower_byte;
	uint8_t upper_byte;
} msm_camera_pdaf_cal;
