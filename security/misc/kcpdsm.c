...
KCPDSM_SYSFS_CREATE(fps_hal, fps_hal);
KCPDSM_SYSFS_CREATE(ril-daemon, ril_daemon);
KCPDSM_SYSFS_CREATE(ril-daemon2, ril_daemon2);
KCPDSM_SYSFS_CREATE(ril-daemon3, ril_daemon3);
...
static ssize_t dump_list_info(char *buf, int need_pid)
{
	...
	needle += dump_fps_hal_info(needle, need_pid);
	needle += dump_ril_daemon_info(needle, need_pid);
	needle += dump_ril_daemon2_info(needle, need_pid);
	needle += dump_ril_daemon3_info(needle, need_pid);
...
static struct attribute *kcpdsm_attrs[] = {
	...
	&kcpdsm_fps_hal_attr.attr,
	&kcpdsm_ril_daemon_attr.attr,
	&kcpdsm_ril_daemon2_attr.attr,
	&kcpdsm_ril_daemon3_attr.attr,
